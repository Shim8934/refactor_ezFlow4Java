...
<html>
	<head>
	    ...
	    <script>
	        ...
	        var columnName = "<c:out value='${columnName}'/>";
	        ...
	        window.onload = function () {
	            ...
	            if (typeof window.opener.authList == "string") {
	            	authList = JSON.parse(window.opener.authList);
	   				...
	   				authList = window.opener.authList;
	   			}
	        ...
	        var setAuthListToXML = function() {
	        	var authListColumn = authList.filter((e) => e.columnName == columnName);
	        	if (authListColumn.length > 0) {
                    var authListData = authListColumn[0].data;
                    var authListLength = authListData.length;
                    for (var i = 0; i < authListLength; i++) {
                        var pparsingXML = "";
                        var pparsingXML2 = "";
        
                        pparsingXML2 = "<LISTVIEWDATA2><ROWS>";
                        var userId = authListData[i].userId;
                        var userType = authListData[i].userType;
                        var userName = authListData[i].userName;
                        var userName1 = authListData[i].userName1;
                        var userName2 = authListData[i].userName2;
                        // var subDept = authListData[i].subdeptPermitted;
                        var strName = "";
                        ...
                        pparsingXML = pparsingXML + "<DATA6><![CDATA[" + userName1 + "]]></DATA6>";
                        pparsingXML = pparsingXML + "<DATA7><![CDATA[" + userName2 + "]]></DATA7>";
	        ...
	        function OK_Click() {
	            ...
	            for (var i = 0; i < memberListLength;i++) {
	            	...
	            	var memberName1 = memberList[i].getAttribute("data6");
	            	var memberName2 = memberList[i].getAttribute("data7");
                    ...
                    userListDataLength += memberId.length + memberName1.length + memberName2.length + memberType.length + 4;
	            }
	            
	            if (userListDataLength > 450) {
	                alert("<spring:message code='ezBoard.extensionAttr.JIH03' />");
	                return;
	            }
	            
	            var returnData = { "columnName" : columnName,
	                            "data" : popupUserList };
	            
	            // opener의 authList에 데이터 저장
	            var openerAuthList = window.opener.authList;
	            openerAuthList = openerAuthList.filter((e) => e.columnName !== columnName);
	            openerAuthList.push(returnData);
	            window.opener.authList = openerAuthList;
	            
	            //opener의 확장컬럼에 가공된 사용자값 작성
	   			window.opener.$((".authList_div." + columnName)).text(userListText.substring(1));
	        ...
	        function InsertReceiver(pListView) {
	            ...
	            if (m_selectedTree == orglistView) {
		            if (p_ListOrderObject == null || p_ListOrderObject == "") {
		                ...
		                var tempNode = document.getElementById(nodeIdx.NodeID);
		                var strName1 = tempNode.getAttribute("value1");
		                var strName2 =  tempNode.getAttribute("value2");
		                ...
		                if (!bFlag) {
		                    ...
		                    pparsingXML = pparsingXML + "<DATA6>" + MakeXMLString(strName1) + "</DATA6>";
		                    pparsingXML = pparsingXML + "<DATA7>" + MakeXMLString(strName2) + "</DATA7>";
		                ...
		                if (listContentArry != "") {
		                    for (var i = 0; i < listContentArry.length; i++) {
                                ...
                                var strName1 = document.getElementById(listContentArry[i]).getAttribute("_data12");
                                var strName2 = document.getElementById(listContentArry[i]).getAttribute("_data13");
    	            ...
    	            if (arrRows.length > 0) {
    	            	for (var i = 0; i < arrRows.length; i++) {
    	                	...
    	                	var strName1 =  GetAttribute(arrRows[i], "data6");
    	                	var strName2 =  GetAttribute(arrRows[i], "data7");
	                        ...
	                        if (bFlag) {
        	                    ...
        	                    pparsingXML = pparsingXML + "<DATA5>N</DATA5>";
                                pparsingXML = pparsingXML + "<DATA6>" + strName1 + "</DATA6>";
                                pparsingXML = pparsingXML + "<DATA7>" + strName2 + "</DATA7>";
	                ...
	                if (arrRows.length > 0) {
	                    ...
	                    for (var i = 0; i < arrRows.length; i++) {
	                        ...
	                        targetList.LoadFromID(listid);
                                ...
                                pparsingXML = pparsingXML + "<DATA7>" + strName + "</DATA7>";
		                ...
		                <table id="listType1">
		                    <tr id="ListMsgTo">
		                        ...
		                        <td style="vertical-align: top;">
		                            ...
		                            <div class="receiver_borderbox" style="height:498px;">
