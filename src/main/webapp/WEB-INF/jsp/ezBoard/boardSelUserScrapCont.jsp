...
<html>
<head>
    <title><spring:message code='ezBoard.kmh48'/></title>
    ...
    <script type="text/javascript" id="clientEventHandlersJS">
        ...
        function btnOK_onclick() {
            ...
            if (nodeIdx == "" || nodeIdx == undefined) {
                OpenAlertUI("<spring:message code='ezBoard.kmh50'/>");
                return;
            }
            $.ajax({
				type : "GET",
				dataType : "JSON",
				async : true,
				url : "/ezBoard/setUserScrapItemList.do",
				data : {
					itemListID : itemID,
					boardID : boardID,
					userScrapContID : treeNode.GetNodeData("DATA1") 
				},
				success: function(map){
					if(map.result == "true"){
							if(map.OverlapItemCount > 0){
								var pAlertContent = "<spring:message code='ezBoard.kmh44'/> [" + map.OverlapItemCount + "]<spring:message code='ezBoard.kmh45'/>";
								ScrapTreeViewRefresh();
                                try {opener.parent.left.ScrapTreeViewRefresh();}catch (e) {}
		                        nodeIdx = undefined;
		                        OpenAlertUI(pAlertContent,btnCancel_onclick);
							}else{
								var pAlertContent = "<spring:message code='ezBoard.kmh47'/>";
								ScrapTreeViewRefresh();
                                try {opener.parent.left.ScrapTreeViewRefresh();}catch (e) {}
		                        nodeIdx = undefined;
		                        OpenAlertUI(pAlertContent,btnCancel_onclick);
						...
						var pAlertContent = "<spring:message code='ezBoard.kmh46'/>";
                    	OpenAlertUI(pAlertContent,btnCancel_onclick);
					}
				}
			});
...
<body class="popup">
    <h1><spring:message code='ezBoard.kmh51'/></h1>
    <div id="close">
        <ul>
            <li><span onclick="return btnCancel_onclick()"></span></li>
        </ul>
    </div>
    <div class="box" style="WIDTH: 480px; HEIGHT: 340px; BACKGROUND-COLOR: #fff; overflow: auto;" id="divUserScrapContTree"></div>
    <div class="btnposition btnpositionNew">
        ...
        <a class="imgbtn" onclick="return btnOK_onclick()"><span><spring:message code='ezBoard.kmh43'/></span></a>
    </div>
