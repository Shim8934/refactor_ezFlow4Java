...
<HTML>
	<HEAD>
        ...
        <script type="text/javascript" src="/js/ezemail/js_cross/TreeView.js"></script>
        <script type="text/javascript" src="/js/ezemail/js_cross/ListView_list.js"></script>
        <script type="text/javascript" src="/js/ezemail/js_cross/Common.js"></script>
        <script type="text/javascript" src="/js/ezemail/js_cross/NameControl.js"></script>
		<title>"${_title}"</title>
		<script language="javascript">
	        var pStartTime = "${_starttime}";
	        var pEndTime = "${_endtime}";
	        var pGubun = "${_gubun}";
	        var type = "${_type}";
	        var bSearch = false;
	        ...
	        function event_displayUserList() {
	            if (g_xmlHTTP != null && g_xmlHTTP.readyState == 4) {
	                if (g_xmlHTTP.statusText == "OK") {
	                    ...
	                    alert("<spring:message code='ezSchedule.t2' />" + g_xmlHTTP.statusText)
	        ...
	        function infoview_click() {
	            ...
	            if (CrossYN())
	                window.open("/myoffice/common/ShowPersonInfo_cross.aspx?id=" + id + "&dept=" + dept, "", "height=450px,width=420px,  top=" + pTop.toString() + ", left=" + pLeft.toString() + ", status = no, toolbar=no, menubar=no,location=no, resizable=1");
	            else
	                window.open("/myoffice/common/ShowPersonInfo.aspx?id=" + id + "&dept=" + dept, "", "height=450px,width=420px, top=" + pTop.toString() + ", left=" + pLeft.toString() + ", status = no, toolbar=no, menubar=no,location=no, resizable=1");
	        }
		    ...
		    function DisplayUserImageList() {
		        ...
		        if (pListType == "IMG") {
		            ...
		            if (pSeach) {
		                document.getElementById("SelectDeptNM").innerHTML = "<img src=\"/images/OrganTree_cross/ic-open.gif\" style=\"vertical-align:middle;\" >" + strLang257 + "" + "-[<span style='color:#017BEC;'>" + getNodeText(SelectNodes(xmlRtn, "LISTVIEWDATA/TOTALCOUNT")[0]) + strLang256 + "</span>]";
		                SelectDeptNM.setAttribute("countinfo", "1")
		            }
		        }
		        else {
		        ...
		        for (var i = 0; i < SelectNodes(xmlRtn, "LISTVIEWDATA/ROWS/ROW").length; i++) {
		            if (pListType == "IMG") {
		                ...
		                Sub_TD2.innerHTML = ReplaceText(M_TR.getAttribute("_DATA5"), "&", "&amp;");
		                ...
		                if (pSeach) {
		                    ...
		                    M_TR_TD1.innerHTML = ReplaceText(M_TR.getAttribute("_DATA5"), "&", "&amp;");
		        ...
		        if (g_xmlHTTP != null && g_xmlHTTP.readyState == 4) {
		            if (g_xmlHTTP.statusText == "OK") {
		                if (g_xmlHTTP.responseXML.getElementsByTagName("ROW").length == 0)
		                    alert("<spring:message code='ezSchedule.t297' />");
		                else {
		                ...
		                alert("<spring:message code='ezSchedule.t9' />" + g_xmlHTTP.statusText)
