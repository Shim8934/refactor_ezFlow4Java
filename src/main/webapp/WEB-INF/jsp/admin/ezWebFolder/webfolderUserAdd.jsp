	...
	<head>
	    ...
	    <script>
	        ...
	        window.onload = function () {
	            ...
	            var openerTarget 			= window.opener.authList${folderManager eq 1? "Manager" : folderManager eq 0? "User" : ""};
	            var openerStrictAuthList 	= window.opener.strictAuthList${folderManager eq 1? "Mng" : folderManager eq 0? "" : ""};
	            if (typeof openerTarget == "string") {
	            	authList = JSON.parse(openerTarget);
	   			} else {
	   				authList = JSON.parse(JSON.stringify(openerTarget));
	   			}
	            if (openerStrictAuthList) {
		            if (typeof openerStrictAuthList == "string") {
		            	strictAuthList = JSON.parse(openerStrictAuthList);
		            } else {
		            	strictAuthList = JSON.parse(JSON.stringify(openerStrictAuthList));
		            }
	            }
	        ...
	        var setAuthListToXML = function() {
	        	...
	        	for (var i = 0; i < authListLength; i++) {
	                ...
	                var manager = Boolean(authList[i].folderManager);
	                var displayDeptName = authList[i].displayDeptName;
	                var strName = "";
	                
	                if (userType == "USER") {
	                	strName = userName + "(" + displayDeptName + ")";
	                ...
	                pparsingXML = pparsingXML + "<DATA6><![CDATA[" + manager + "]]></DATA6>";
	                pparsingXML = pparsingXML + "<DATA7><![CDATA[" + displayDeptName + "]]></DATA7>";
	        ...
	        function displayUserList(DeptID) {
	        	...
	        	$.ajax({
		        	...
		        	async : true,
		        	data : {deptID : DeptID, cell : "company;description;displayName;title;telephoneNumber", prop : "mail;displayName;description;title;company;telephoneNumber;extensionAttribute2;department", type : "user",  adminOrgan : "y"},
...
// 	                    headerData = loadXMLString(listviewheader.innerHTML.toUpperCase());
		        	...
		        	error : function(error){
		        		/* 2020-12-15 김은실 - [카이스트] 회사 폴더별 관리자 지원 기능: 빠르게 닫을 때 버그로 보일 가능성: 그냥 잘 닫히도록 수정.
	        ...
	        function search_click() {
				...
				if(document.all("search_type").value == "description" ) {
					deptsearch_click();
				} else {
...
// 	                    headerData = loadXMLString(listviewheader.innerHTML.toUpperCase());
	        ...
	        function deptsearch_click() {
	            ...
	            var searchWord = document.all("keyword").value.trim();
	            // whitespace를 제거한 스트링으로 Text Box를 다시 세트한다.
	            document.all("keyword").value = searchWord;
	            /* 
	            if (searchWord == "") {
	                alert("<spring:message code='ezEmail.t10' />");
	            ...
	            if (adCount == 0) {
	                ...
	                if (CrossYN()) {
	                    var strQuery = "<DATA><DEPTID>" + xmlDOM.getElementsByTagName("DATA2").item(0).textContent + "</DEPTID><TOPID>Top</TOPID><PROP></PROP><DISPLAYTRASHDEPT>true</DISPLAYTRASHDEPT><ADMINORGAN>y</ADMINORGAN></DATA>";
	                } else {
	                    var strQuery = "<DATA><DEPTID>" + xmlDOM.getElementsByTagName("DATA2").item(0).text + "</DEPTID><TOPID>Top</TOPID><PROP></PROP><DISPLAYTRASHDEPT>true</DISPLAYTRASHDEPT><ADMINORGAN>y</ADMINORGAN></DATA>";
	                }
	                ...
	                if (CrossYN()) {
	                    ...
	                    if (rgParams["deptid"] != "") {
	                        bSearch = true;
	                        g_xmlHTTP = createXMLHttpRequest();
	                        var strQuery = "<DATA><DEPTID>" + rgParams["deptid"] + "</DEPTID><TOPID>Top</TOPID><PROP>mail</PROP><DISPLAYTRASHDEPT>true</DISPLAYTRASHDEPT><ADMINORGAN>y</ADMINORGAN></DATA>";
	                        g_xmlHTTP.open("POST", "/ezOrgan/getDeptTreeInfo.do", true);
	                        g_xmlHTTP.onreadystatechange = event_getDeptFullTree;
	                        g_xmlHTTP.send(strQuery);
	                    }
	        ...
	        function OK_Click() {
	            ...
	            if (${folderManager ne 1} && memberListLength <= 0) {
	            ...
	            for (var i = 0; i < memberListLength;i++) {
	            	...
	            	var displayDeptName = memberList[i].getAttribute("data7");
	            	...
	            	popupUserList.push({"userId" : memberId, "userName" : memberName, "userType" : memberType, "subdeptPermitted" : subDept, 
	            		"folderManager" : Boolean(${folderManager}), "sn" : i , "displayDeptName":displayDeptName});
	            }

		     	// 2020-12-04 김은실 - (카이스트)회사 폴더별 관리자 지원 기능 
           		window.opener.authList${folderManager eq 1? "Manager" : folderManager eq 0? "User" : ""} = JSON.stringify(popupUserList);
           		webfolderAddUserList(popupUserList);
           		
           		var folderMng = '${folderManager}';
           		if (!folderMng || arr == ''){
           			
        			// 2020-12-10 김은실 - (카이스트)회사 폴더별 관리자 지원 기능  :정리
           			if(Boolean("${folderManager}")){
           				// 웹폴더
			   			window.opener.addUser${folderManager eq 1? "Manager" : ""} = JSON.stringify(addUser);
			   			window.opener.deleteUser${folderManager eq 1? "Manager" : ""} = JSON.stringify(deleteUser);
           			}
         			if(window.opener.$("#folderManagerTR").length > 0){
			   			// 사용자화면 :담당자
			   			//window.opener.saveChanges_manager();
		   				var saveMsg = window.opener.saveChanges_manager(true);
         				alert(saveMsg);
         			}else{
		   				// 웹폴더도 popupUser페이지를 공유함.
			   			window.opener.$("${folderManager eq 1? '#newTargetDivManager' : folderManager eq 0? '#newTargetDiv' : '#authList_div'}").text(userListText.substring(1));
         			}
	        ...
	        function closeAllPopup() {
	        	var blockLeft = document.getElementById("mailPanel");
	        ...
	        function returnFunction(type){
	   			window.opener.subFolderType= type;
	        ...
	        var webfolderAddUserList_parm = new Array();
	        // 웹폴더 추가된 사용자 리스트 구해서 출력해주는 로직 추가 
	        function webfolderAddUserList(popupUserList){
	        	arr = [];
	        	var beforeUser = strictAuthList.map(function(popup,index){return popup.userId});
	        	var afterUser = popupUserList.map(function(popup,index){return popup.userId});
	        	deleteUser = beforeUser.filter(function(z){return afterUser.indexOf(z)<0;});// 삭제
	        	addUser = afterUser.filter(function(z){return beforeUser.indexOf(z)<0;});// 추가
	        	var AllList = $.extend({},strictAuthList,popupUserList);
	        	...
	        	if (addUser.length != 0){
	        		...
	        		for(var i =0; i<addUser.length;i++){
		        		var index = afterUser.indexOf(addUser[i]);
		        		temp.push(index);
	        		};
	        		
	        		for(var j =0; j<temp.length;j++){
						userInfo.push(popupUserList[temp[j]]);
	        		};

	        		for(var i =0 ; i<userInfo.length;i++){
		        		if(arr.indexOf(userInfo[i].userId)<0){
			        		var temp = new Object();
			        		temp.username = userInfo[i].userName;
			        		temp.userid = userInfo[i].userId;
			        		arr.push(temp);
		        		}
		        	}
	        		...
	        		webfolderAddUserList_parm[0] = DivPopUpHidden;
	        		webfolderAddUserList_parm[1] = arr;
	        		webfolderAddUserList_parm[2] = returnFunction;
	        		
	        		// 2020-12-10 김은실 - (카이스트)회사 폴더별 관리자 지원 기능  : 사용자화면(:담당자)
	        		if(window.opener.$("#folderManagerTR").length > 0){
		 	            DivPopUpShow(400, 300, "/admin/ezWebFolder/webfolderAddMemberList.do?isManager=true");
	        		} else {
	 	            	DivPopUpShow(400, 300, "/admin/ezWebFolder/webfolderAddMemberList.do");
	        		}
	        	}
	        }
		            ...
		            if (p_ListOrderObject == null || p_ListOrderObject == "") {
		                ...
		                if (listContentArry != "") {
		                    for (var i = 0; i < listContentArry.length; i++) {
		                        ...
		                        var deptName = document.getElementById(listContentArry[i]).getAttribute("_data5");
		                        ...
		                        var bFlag = getlistview.ExistRow("data1", strId);
		                     	...
		                     	// 중복체크 추가
		                        if (bFlag) {
									alert(strName + "(" + deptName + ")" + ": <spring:message code='ezBoard.t20' />");
								} else {
		                            pparsingXML2 = "";
		                            pparsingXML = "";
		                            pparsingXML2 = "<LISTVIEWDATA2><ROWS>";
		                            ...
		                            pparsingXML = pparsingXML + "<DATA5>N</DATA5>";
		                            pparsingXML = pparsingXML + "<DATA7>"+MakeXMLString(deptName)+"</DATA7>";
	        ...
	        function DeleteAllReceiver(pListView) {
        		var listid = "MsgToList";
	            var selList = new ListView();
	            selList.LoadFromID(listid);
	        	
	            var arrRows = selList.GetDataRows();
	            $.each(arrRows, function(i, e) {
	            	 selList.DeleteRow(e.id);
	            });
	        }
	        ...
	        function SelectReceiverWindow() {
				...
				if (arrRows == "") {
					...
					if (arrRows[0].getAttribute("DATA4") == "ORGAN" || arrRows[0].getAttribute("DATA4") == "DEPT") {
						...
						if (document.getElementById("admin_OK") != null) {
		            ...
		            <td style="vertical-align: top;">
		            	<div class="portlet_tabpart01" style="margin:0px;">
		            		<div class="portlet_tabpart01_top" id="tab1" style="margin-bottom:3px;">
	            				...
	            				<!-- 2020-11-27 김은실 - (카이스트)회사 폴더별 관리자 지원 기능  -->
		            			<p id="orgTabButton2" style="${folderManager != 0 ? 'display: none;' : ''}">
		            			...
		            			<p id="orgTabButton3" style="${folderManager != 0 ? 'display: none;' : ''}">
		            			...
		            			<p id="orgTabButton4" style="${folderManager != 0 ? 'display: none;' : ''}">
		                                        ...
		                                        <tr>
		                                            <td>
		                                                <div style="margin-left: 5px;">
		                                                    ...
		                                                    <input id="keyword" value="" onkeypress="search_press(event)" onmousedown="keyword_Clear();" style="width: 144px; margin: 0px;height:24px">
		                                            ...
		                                            <td>
		                                                <div style="float: right; margin-right: 5px; position: relative;">
		                                                	<!-- 2020-11-27 김은실 - (카이스트)회사 폴더별 관리자 지원 기능 -->
		                                                    <a class="imgbtn" id="dept_select" style="${folderManager eq 1? 'display: none;' : ''}"><span onclick="dept_select()" style="z-index:10"><spring:message code='ezEmail.t596' /></span></a>
		                ...
		                <table id="ListViewGroupTD" style="display: none">
		                    <tr>
		                        <td>
		                            ...
		                            <div style="width: 668px; height: 464px; overflow: auto; background-color: #ffffff; margin-top: 3px;" id="ListViewGroup" class="border_gray">
		                            </div>
		                        </td>
		                    </tr>
	                	</table>
		                    ...
		                    <tr id="ListMsgTo">
		                        ...
		                        <td style="vertical-align: top;">
		                            ...
		                            <div class="receiver_borderbox">
		                                <div id="ListViewMsgTo" ondragover ="onDragEnter(event, this)" ondrop ="onDrop(event, this)" style="width: 250px; Height: 491px; border-top:1px solid #ddd; overflow: auto;" ondblclick="DeleteReceiver(ListViewMsgTo)"></div>
		                            </div>
		                            <table class="content" style="display:none;width: 100%;">
						            	<tbody>
						                	<tr>
						                    	<th>
						                    	<c:if test="${empty folderManager}">
						                    		<spring:message code='ezBoard.t999025' />
						                    	</c:if>
						                    	</th>
						                    	<td>
						                    	<c:if test="${empty folderManager}">
							                        <input type="checkbox" id="admin_OK" name="admin_OK" onclick="checkbox_onclick(event)" disabled>&nbsp;<spring:message code='ezSurvey.t51' />
							                        <input type="checkbox" id="admin_NO" name="admin_NO" onclick="checkbox_onclick(event)" disabled>&nbsp;<spring:message code='ezSurvey.t50' />
						                    	</c:if>
							                    </td>
						                	</tr>
						            	</tbody>
