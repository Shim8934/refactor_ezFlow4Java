...
Ajax.swfHTTPRequest = Class({
	...
	__init : function() {
	    this._tmpId = 'tmpSwf' + (new Date).getMilliseconds() + Math.floor(Math.random((new Date).getMilliseconds()) * 100000);
	},
...
Ajax.initFlash = function(swf_path) {
	...
	Ajax._tmpId = 'tmpSwf'+(new Date).getMilliseconds()+Math.floor(Math.random((new Date).getMilliseconds())*100000);
...
var Element = {
	...
	/** @id Element.hide */
	hide : function() {
		[].load(arguments).each(function(v){ $B(v).style.display='none'; });
	},
	/** @id Element.toggle */
	toggle : function() {
	...
	visible : function(oEl) {
		return (this.getCSS($B(oEl), 'display')!='none');
	},
	/** @id Element.has */
	...
	addClass : function(oEl, className) {
		if (!this.hasClass(oEl, className)) $B(oEl).className = ($B(oEl).className+' '+className).replace(/^\s+/,'');
	},
	/** @id Element.removeClass */
	removeClass : function(oEl, className) {
		for (var i = 1, className; className = arguments[i]; i++)
			$B(oEl).className = $B(oEl).className.replace(new RegExp('\\b' + className + '(\\s+|$)', 'g'), '');
	},
...
document.getElementsByClassName = function(className, oParent) {
	var a = [].load(($B(oParent) || document.body).getElementsByTagName('*'));
...
Ju.Static = Class({
	...
	onScroll : function(e) {
		...
		if (this.options.limitArea) {
			...
			var area = $B(this.options.limitArea);
...
Ju.controlLayer = Class({
	...
	hide : function(sLayerID){
		if(sLayerID) this._oElement = $B(sLayerID);
...
Ku.AutoExpand = Class({
	__init : function(id) {
		...
		this.obj.parentNode.insertBefore(this.imgGrip, this.obj);
		...
		this.setGripBottomSpacing();		
		this.setGripRightSpacing();
		...
		AutoExpandList[AutoExpandList.length] = this;
	...
	setGripBottomSpacing : function(){
		this.gripBottomSpacing = 12 + this.options.gripImg_y_spacing;
	},
	setGripRightSpacing : function(){
		if($Agent().IE){
			this.gripRightSpacing = 14 + this.options.gripImg_x_spacing;
		}else{
			this.gripRightSpacing = 15 + this.options.gripImg_x_spacing;
		}
		
		if(this.obj.style.overflowY != 'hidden'){
			this.gripRightSpacing += 17;
		}
	},
	evaluate : function() {
		...
		if (this.clone.scrollHeight > this.obj.clientHeight) {
			while(this.clone.scrollHeight > this.obj.clientHeight && this.obj.rows < this.options.maxRows && this.obj.rows < this.options.stopAutoExpandAt) {
				this.obj.rows++;
			}
		}else {
			while(this.clone.scrollHeight+5 < this.obj.clientHeight && this.obj.rows > this.options.minRows && this.obj.rows < this.options.stopAutoExpandAt) {
				this.obj.rows--;
			}
		}
...
/*		
		if (this.obj.rows < this.options.maxRows) {
			this.obj.style.overflowY = 'hidden';
			if($Agent().IE) this.obj.rows++;
		} else {
			this.obj.style.overflowY = 'scroll';
		}
*/		
...
//document.getElementById("search").value += ',d'+e.clientY;
	...
	onGripMouseUp:function(e){
		...
		this.setGripRightSpacing();
		this.draw();
//document.getElementById("search").value += ',u'+e.clientY;
		e.stop();
	},
	onGripMouseMove:function(e){
		var pos = this.realPos(this.obj);
		var rows = Math.floor((e.clientY-pos.top+(20-this.imgGrip_offsetY))/this.rowHeight)-($Agent().IE?0:1);
		if(rows < 1) rows = 1;
		if(rows > this.options.maxRows) rows = this.options.maxRows;

		this.obj.rows = rows;
		this.draw();
//document.getElementById("search").value = pos.top+','+e.clientY+','+(e.clientY-pos.top+(20-this.imgGrip_offsetY));
		e.stop();
	},
	drawSingleObj:function(){
		var pos = this.realPos(this.obj);
//		alert(this.obj.style.top);
//		alert(pos.top+', '+pos.left);
		this.imgGrip.style.top = (pos.top + this.obj.clientHeight - this.gripBottomSpacing)+ 'px';
		this.imgGrip.style.left = (pos.left + toInt(this.obj.style.width) - this.gripRightSpacing) + 'px';
	},
	draw:function(){
		for(var i=0; i<AutoExpandList.length;i++){
			AutoExpandList[i].drawSingleObj();
		}
	}
});
/**
...
Ku.Upload = Class({
	...
	_getTmpId : function() {
	    return (new Date).getMilliseconds() + Math.floor(Math.random((new Date).getMilliseconds()) * 100000);
	},
...
Ku.Upload.initFlash = function(swf_path) {
	...
	Ku.Upload._tmpId = 'tmpSwf' + (new Date).getMilliseconds() + Math.floor(Math.random((new Date).getMilliseconds()) * 100000);
...
Ku.Selectbox = Class({
    __init : function(id) {
        ...
        with(this._txt_element) {
            ...
            style.marginLeft = 0;//style.marginTop;
        }
...
Ku.Scrollbox = Class({
	...
	Ku.SetClipboard.initFlash = function(sSwfPath){
		...
		Ku.SetClipboard._sModuleId = 'SetClipboardModule' + (new Date).getMilliseconds() + Math.floor(Math.random((new Date).getMilliseconds()) * 100000);
...
Ku.Slider = Class({
	...
	__init : function(id) {
		...
		var u = this._unit_element = $B(o.unit);
	...
	setValue : function(v) {
		...
		if (this.options.vertical) {
			...
			this._unit_element.style.left = '0px';
		}
