...
function showProgress() {
    g_progresswin = modelessWindow("/myoffice/ezApproval/show_progress_Cross.aspx?fileinfo=" + escape(strLangS313), "", 390, 185, g_progresswin);
}
...
function MakeAprLineListXML (pAutoRuleGuid) {
    ...
    pListXml = pListXml + "<HEADER><NAME>" + strLangS230 + "</NAME><WIDTH>30</WIDTH></HEADER>";
    pListXml = pListXml + "<HEADER><NAME>" + strLangS107 + "</NAME><WIDTH>50</WIDTH></HEADER>";
    pListXml = pListXml + "<HEADER><NAME>" + strLangS49 + "</NAME><WIDTH>60</WIDTH></HEADER>";
    pListXml = pListXml + "<HEADER><NAME>" + strLangS108 + "</NAME><WIDTH>80</WIDTH></HEADER>";
    pListXml = pListXml + "<HEADER><NAME>" + strLangS38 + "</NAME><WIDTH>80</WIDTH></HEADER>";
    pListXml = pListXml + "<HEADER><NAME>" + strLangS109 + "</NAME><WIDTH>80</WIDTH></HEADER>";
    pListXml = pListXml + "<HEADER><NAME>" + strLangS231 + "</NAME><WIDTH>80</WIDTH></HEADER>";
    pListXml = pListXml + "</HEADERS>";
    ...
    for (var i = 0; i < GetElementsByTagName(AprLineXML, "ROW").length; i++) {
        if (getNodeText(GetElementsByTagName(AprLineXML, "AUTORULEGUID")[i]) == pAutoRuleGuid) {
            ...
            switch (getNodeText(GetElementsByTagName(AprLineXML, "APRMEMBERID")[i])) {
                case "#boss":
                    pListXml = pListXml + "<VALUE>" + strLangS1115 + "</VALUE>";
                    break;
                case "#upboss":
                    pListXml = pListXml + "<VALUE>" + strLangS1116 + "</VALUE>";
                    break;
                case "#upupboss":
                    pListXml = pListXml + "<VALUE>" + strLangS1117 + "</VALUE>";
                    break;
                case "#upupupboss":
                    pListXml = pListXml + "<VALUE>" + strLangS1118 + "</VALUE>";
                    break;
                case "#upupupupboss":
                    pListXml = pListXml + "<VALUE>" + strLangS1119 + "</VALUE>";
                    break;
                case "#drafter":
                    pListXml = pListXml + "<VALUE>" + strLangS514 + "</VALUE>";
                    break;
                case "#selboss":
                    pListXml = pListXml + "<VALUE>" + strLangS1120 + "</VALUE>";
                    break;
                default:
...
function MakeHeader() {
    pparsingXML = "<HEADERS>";
    pparsingXML = pparsingXML + "<HEADER><NAME>" + strLangS230 + "</NAME><WIDTH>30</WIDTH></HEADER>";
    pparsingXML = pparsingXML + "<HEADER><NAME>" + strLangS107 + "</NAME><WIDTH>50</WIDTH></HEADER>";
    pparsingXML = pparsingXML + "<HEADER><NAME>" + strLangS49 + "</NAME><WIDTH>60</WIDTH></HEADER>";
    pparsingXML = pparsingXML + "<HEADER><NAME>" + strLangS108 + "</NAME><WIDTH>80</WIDTH></HEADER>";
    pparsingXML = pparsingXML + "<HEADER><NAME>" + strLangS38 + "</NAME><WIDTH>80</WIDTH></HEADER>";
    pparsingXML = pparsingXML + "<HEADER><NAME>" + strLangS109 + "</NAME><WIDTH>80</WIDTH></HEADER>";
    pparsingXML = pparsingXML + "<HEADER><NAME>" + strLangS231 + "</NAME><WIDTH>80</WIDTH></HEADER>";
...
function AddAprLine_Dept(pCurSelectedRow) {
    ...
    if (pCurSelectedRow == null) {
        ...
        if (chkDuplflag) {
            var pInformationContent = strLangS227;
            var Ans = OpenInformationUI(pInformationContent);
//            if (!Ans) {
            return;
//            }
