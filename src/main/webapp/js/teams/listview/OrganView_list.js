...
function OrganView() {
    ...
    //tr_select 사용 여부
    this.SetCheckBoxOnly = SetCheckBoxOnly;
    ...
    function SetCheckBoxOnly(flag) {
        _isCheckBoxOnly = flag;
    }
    ...
    function LoadFromID(pTableID) {
        ...
        strAttribute = GetAttribute(oList, "usecheckboxonly");
        if (strAttribute == "true")
            _isCheckBoxOnly = true;
        else
            _isCheckBoxOnly = false;
    ...
    function DataBind(pTagetID) {
        ...
        if (objElm) {
            ...
            oTable.setAttribute("usecheckboxonly", _isCheckBoxOnly);
    ...
    function GetTableHeaderObj() {
        ...
        for (var i = 0; i < oHeaders.length; i++) {
            ...
            if (strWidth != "0") { //2011.07.05 Header의 width가 0이면 td를 만들지 않는다.
                ...
                if (strClass != "") {
                    ...
                    //objTd.setAttribute("bgcolor", "#f0f0f0");
                }
                ...
                if (strColName == "ITEMID") {
                    ...
                    _HeaderCheckBox.onclick = new Function("Check_AllRows()");
        ...
        objTheader.style.position = "sticky";
        objTheader.style.top = 0; objTheader.style.zIndex = 10;
    ...
    function GetTableBodyObj() {
        ...
        if (oRows.length == 0) {
            ...
            var oText = document.createTextNode(strLang41);
        ...
        */
        ...
        for (var i = 0; i < oRows.length; i++) {
            ...
            for (var j = 0; j < oHeaders.length; j++) {
                strValue = SelectSingleNodeValue(oCells[j], "VALUE");
                ...
                //objTd.style.paddingTop = "1px";

                if (SelectSingleNodeValue(oHeaders[j], "COLNAME") == "ITEMID") {
                    //objTd.style.textAlign = "left";
                    //objTd.style.height = "55px";

                    var _TDCheckBox_Sub = document.createElement("INPUT");
                    _TDCheckBox_Sub.type = "checkbox";
                    _TDCheckBox_Sub.id = strValue;
                    _TDCheckBox_Sub.name = "chkbox";
                    _TDCheckBox_Sub.setAttribute("style", "width: 13px; height: 13px; padding-top: 0px; padding-right: 0px; padding-bottom: 0px; padding-left: 0px; margin-top: 0px; margin-right: 0px; margin-bottom: 0px; margin-left: 0px;");
                    _TDCheckBox_Sub.onclick = new Function("Check_Row(this)");
                    objTd.appendChild(_TDCheckBox_Sub);
                }
                else if (SelectSingleNodeValue(oHeaders[j], "COLNAME") == "PHOTO") {
                    //objTd.style.textAlign = "left";
                    //objTd.style.verticalAlign = "top";
                    //objTd.style.height = "55px";
                    //objTd.style.paddingTop = "2px";
                    if (usePresence.toUpperCase() == "Y") {
                        objTd.innerHTML = "<span><img onclick='nameCard(this)' style='height:50px;' src='/images/Presence/Lync/PresenceControlOFF.gif' /></span>";
                    } else {
                        objTd.innerHTML = "<span style='display:none;'><img onclick='nameCard(this)' style='height:50px;' src='/images/Presence/Lync/PresenceControlOFF.gif' /></span>";
                    }

                    objTd.innerHTML += "<span><img onclick='nameCard(this)'  src='' style='cursor:pointer; width:50px; height:50px; border:1px solid #b6b6b6;' onerror='src=\"/images/teams/NoPhoto.png\"'/></span>";
                }
                else if (SelectSingleNodeValue(oHeaders[j], "COLNAME") == "NAMECARD") {
                    sipuri = strValue.substring(strValue.lastIndexOf("E-MAIL :")).split(" : ")[1];
                    var IsEmail = strValue.indexOf("E-MAIL :");
                    // 메일 주소 클릭 시 쓰기 창 띄워지도록 처리
                    if (IsEmail > -1)
                        strValue = strValue.replace("E-MAIL :", "E-MAIL : <span style='cursor:pointer;color:Blue;' onclick='MailSend(this)' title='메일쓰기'>") + "</span>";
                    /*
                    else
                        objTd.setAttribute("ondblclick", "btn_OpenConversation('" + sipuri + "')");
                    */

                    objTd.setAttribute("ondblclick", "btn_OpenConversation('" + sipuri + "')");

                    //objTd.style.textAlign = "left";
                    //objTd.style.paddingLeft = "20px";
                    //objTd.style.height = "55px";

                    objTd.innerHTML = strValue;
                }
                else if (SelectSingleNodeValue(oHeaders[j], "COLNAME") == "DELETE") {
                    objTd.innerHTML = "<span><img id='" + objTr.id + "' onClick='delete_onClick(this)' src='/images/btn_close.gif' /></span>";
                }
                else if (SelectSingleNodeValue(oHeaders[j], "COLNAME") == "DELETEADMIN") {
                    objTd.innerHTML = "<span><img id='" + objTr.id + "' onClick='remove_member_onClick(this)' src='/images/btn_close.gif' /></span>";
                }
                else {
                    //objTd.style.textAlign = "left";
                    //objTd.style.height = "20px";
    ...
    //리스트뷰 바디의 Row 중 ID 로 데이터 가져오기
    function GetDataRow(pId) {
        var elements = new Array();
        var arrRow = GetDataRows();
        ...
        for (var i = 0; i < length; i++) {
            var strAttribute = GetAttribute(arrRow[i], "id");
            if (strAttribute == pId) {
...
function tr_unselectedAll(pTableID) {
    ...
    if (!document.getElementById("HeaderAllCheckBox").checked) {
        var SelList = new OrganView();
        SelList.LoadFromID(pTableID);
