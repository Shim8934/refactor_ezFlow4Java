...
{
	var color = showModalDialog("Controls/manyColor.aspx", null, "dialogHeight:290px; dialogWidth:286px; status:no;scroll:no; help:no; edge:sunken");
...
{    
	...
	ContextMenu[0] = new ContextMenuItem("잘라내기", DECMD_CUT);
	ContextMenu[1] = new ContextMenuItem("복사하기", DECMD_COPY);
	ContextMenu[2] = new ContextMenuItem("붙여넣기", DECMD_PASTE);
	ContextMenu[3] = new ContextMenuItem("지우기", DECMD_DELETE);
	ContextMenu[4] = new ContextMenuItem(MENU_SEPARATOR, 0);
	ContextMenu[5] = new ContextMenuItem("전체선택", DECMD_SELECTALL);
}

// 2007.04.13.이창호
function tbContentElement_DocumentComplete()
{
    //tbContentElement.editor.DOM.body.free = "";
    tbContentElement.Editor.DOM.body.setAttribute("free", "");
	tbContentElement.editor.DOM.body.topMargin = "7px";
	tbContentElement.editor.DOM.body.leftMargin = "7px";
	tbContentElement.editor.DOM.charset = "ks_c_5601-1987";
	tbContentElement.editor.DOM.body.setAttribute("free", " ");
	retStyle = tbContentElement.editor.DOM.createStyleSheet("", 0);
	retStyle.addRule("P", "margin-top:2px; margin-bottom:2px;"); 

	var eBody = "";
	eBody = "<div style=\"font-family: 굴림;word-break:break-all\"><div>" + eBody + "</div></div>";
	tbContentElement.editor.DOM.body.innerHTML = eBody;
...
function tbContentElement_DocumentComplete()
{
	tbContentElement.DOM.charset = "ks_c_5601-1987";
	tbContentElement.DOM.body.style.FontFamily = "굴림";
	tbContentElement.DOM.body.style.FontSize = "10pt";

	retStyle = tbContentElement.DOM.createStyleSheet("", 0);
	retStyle.addRule("P", "margin-top:2px; margin-bottom:2px;"); 
	tbContentElement.ExecCommand(DECMD_SETFONTSIZE, OLECMDEXECOPT_DODEFAULT, 2);

	var eBody = sigBody.innerHTML;
	eBody = "<div style=\"font-size: 10pt; font-family: 굴림\"><div>" + eBody + "</div></div>";
	tbContentElement.DOM.body.innerHTML = eBody;
	var retVal = tbContentElement.DOM.body.createTextRange();
	retVal.collapse(true);
	retVal.select();
}
*/
