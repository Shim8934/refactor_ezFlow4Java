...
function transmission_mail_onclick() {
    if (listContentArry.length == 0 && listSubContentArry.length == 0) {
        ...
        var pURI;
        if (CrossYN() || pNoneActiveX == "YES") {
            pURI = "mail_write_Cross.aspx?cmd=FORWARD&URL=" + encodeURIComponent(pSelectItem.getAttribute('_href'));
        }
        else {
            if (pUse_Editor == "")
                pURI = "mail_write.aspx?cmd=FORWARD&URL=" + encodeURIComponent(pSelectItem.getAttribute('_href'));
            else
                pURI = "mail_write_IE.aspx?cmd=FORWARD&URL=" + encodeURIComponent(pSelectItem.getAttribute('_href'));
        }        
        var newwin = window.open(pURI, "", "top=" + pTop.toString() + ", left=" + pLeft.toString() + ", height = " + conHeight + "px, width = 890px, status = no, toolbar=no, menubar=no,location=no, resizable=1");
        newwin.focus();
    }
...
function move_mail_onclick() {
    ...
    var moveUrl;
    if (!CrossYN())
        moveUrl = window.showModalDialog("mail_movecopy.aspx", null, "dialogHeight:375px; dialogWidth:320px; status:no; help:no; edge:sunken");
    else
        moveUrl = window.showModalDialog("mail_movecopy_cross.aspx", null, "dialogHeight:375px; dialogWidth:320px; status:no; help:no; edge:sunken");
...
function reject_onclick() {
    ...
    for (var n = 0; n < RejectArray.length; n++) {
        ...
        try {
            xmlHTTP.open("POST", "/myoffice/ezEmail/remote/mail_get_fromemail.aspx", false);
            xmlHTTP.send(xmlpara);
    ...
    var retVal = window.showModalDialog("htm/denial.aspx", params, "dialogWidth:314px; dialogHeight:335px; status:no; help:no; scroll:no; edge:sunken");
    ...
    for (var i = 0; i < retVal.email.length; i++) {
        objRow = createNodeAndAppandNode(xmlpara, objNode, objRow, "ROW");
        createNodeAndAppandNodeText(xmlpara, objRow, objRow2, "DENIAL", retVal.email[i]);
    }
    var xmlHTTP = new XMLHttpRequest();
        ...
        if (xmlhttp_mailPreview.status >= 200 && xmlhttp_mailPreview.status < 300) {
            ...
            for (var Cnt = 0; Cnt < pReceiverArray.length; Cnt++) {
                ...
                if (pReceiver_.length > 10) {
                    ...
                    if (Cnt == 0) {
                        pReceiverHtml = "<span onmouseover=this.style.color='#164aad' onmouseout=this.style.color='#666'  style='cursor:pointer' title='" + ConvertStringForHTML(pReceiver_Name) + "' onclick='show_personinfo(\"" + pReceiver_Address + "\")'>\"" + ConvertStringForHTML(pReceiver_Name) + "\"</span>";
                        
                    }

                    if (pReceiverDetailHtml != "")
                        pReceiverDetailHtml += "&nbsp;,&nbsp;";
                    pReceiverDetailHtml += "<span onmouseover=this.style.color='#164aad' onmouseout=this.style.color='#666'  style='cursor:pointer' title='" + ConvertStringForHTML(pReceiver_Name) + "' onclick='show_personinfo(\"" + pReceiver_Address + "\")'>\"" + ConvertStringForHTML(pReceiver_Name) + "\"</span>";
            ...
            if (pCc != "") {
                ...
                for (var Cnt = 0; Cnt < pCcArray.length; Cnt++) {
                    ...
                    if (pCc_.length > 10) {
                        ...
                        if (Cnt == 0) {
                            pCcHtml = "<span onmouseover=this.style.color='#164aad' onmouseout=this.style.color='#666'  style='cursor:pointer' title='" + ConvertStringForHTML(pCc_Name) + "' onclick='show_personinfo(\"" + pCc_Address + "\")'>\"" + ConvertStringForHTML(pCc_Name) + "\"</span>";

                        }

                        if (pCcDetailHtml != "")
                            pCcDetailHtml += "&nbsp;,&nbsp;";
                        pCcDetailHtml += "<span onmouseover=this.style.color='#164aad' onmouseout=this.style.color='#666'  style='cursor:pointer' title='" + ConvertStringForHTML(pCc_Name) + "' onclick='show_personinfo(\"" + pCc_Address + "\")'>\"" + ConvertStringForHTML(pCc_Name) + "\"</span>";
            ...
            pMailSenderHtml = pOCS + "<span onmouseover=this.style.color='#164aad' onmouseout=this.style.color='#666'  style='cursor:pointer' title='" + ConvertStringForHTML(pFromname) + "' onclick='show_personinfo(\"" + pFromemail + "\")'>\"" + ConvertStringForHTML(pFromname) + "\"</span>";
...
function callMsgDlg(szContentClass, Href) {
    if (szContentClass == "IPM.Appointment") {
        ...
        window.open("/myoffice/ezSchedule/schedule_read.aspx?id=" + encodeURIComponent(scheduleid), "",
                feature);
        return;
    }
    var pheight = window.screen.availHeight;
...
function mail_export() {
    if (!CrossYN()) {
        if (listContentArry.length == 0 && listSubContentArry.length == 0) {
            ...
            var param = { "href": new Array(), "parent": new Object() };
            param["subject"] = new Array();
            param["date"] = new Array();
            for (var i = 0; i < PcSaveArrayList.length; i++) {
                param["href"][i] = PcSaveArrayList[i].getAttribute("_href");
                param["subject"][i] = PcSaveArrayList[i].getAttribute("_subject");
                param["date"][i] = PcSaveArrayList[i].childNodes.item(PcSaveArrayList[i].childNodes.length - 2).innerText
            }
            param["parent"] = window;
            window.showModalDialog("htm/mail_export.aspx", param, "dialogWidth:480px; dialogHeight:250px; scroll:no; status:no; help:no; scroll:no; edge:sunken");
        }
    }
    else {
        if (listContentArry.length == 0 && listSubContentArry.length == 0) {
...
function PopUpPreMail() {
    ...
    if (isMainlist) {
        ...
        if (CurrentPage == 1 && isCurrentCnt == 0) {
            ...
            setTimeout(function () { PagingMove(CurrentPageSize, true); }, 2000);
            return "PREMOVE";
        }
        else {
...
function PopUpNextMail() {
    ...
    if (isMainlist) {
        ...
        if (CurrentPage == CurrentMaxPage && isCurrentCnt == (CurrentMaxCount  - ((CurrentMaxPage - 1) * CurrentPageSize))-1) {
            ...
            listContentArry = new Array();
            listSubContentArry = new Array();
            setTimeout(function () { PagingMove(0); }, 2000);
            return "NEXTMOVE";
        }
        else {
            var preCurrentCnt = isCurrentCnt + 1;
...
function view_recallMessageReport(pnum) {
    ...
    window.open("/myoffice/ezemail/htm/cancelMessageReport.aspx?num=" + pnum, '', feature);
}
