...
function new_mail_onclick(fromE) {
    ...
    pUrl = "/ezEmail/new/mailWrite.do?cmd=NEW" + msgStr;
...
function ReSend(pURL, pEmail) {
    ...
    var requestUrl = "/ezEmail/new/mailWrite.do?url=" + encodeURIComponent(pURL) + "&cmd=RESEND&msgto=" + encodeURIComponent(pEmail);
...
function reply_mail_onclick() {
    ...
    if (listContentArry.length > 1 || listSubContentArry.length > 1) {
        ...
        var pURI = "/ezEmail/new/mailWrite.do?cmd=REPLY&URL=" + encodeURIComponent(pSelectItem.dataset.href);
...
function all_reply_mail_onclick() {
    ...
    if (listContentArry.length > 1 || listSubContentArry.length > 1) {
        ...
        var pTop = (pheight - conHeight) / 2;
        var pLeft = (pwidth - 1200) / 2;
        var pURI = "/ezEmail/new/mailWrite.do?cmd=REPLYALL&URL=" + encodeURIComponent(pSelectItem.dataset.href);
...
function reSend_onClick() {
    ...
    if (listContentArry.length > 1 || listSubContentArry.length > 1) {
        ...
        var pURI = "/ezEmail/new/mailWrite.do?cmd=RESEND&URL=" + encodeURIComponent(pSelectItem.dataset.href);
...
function transmission_mail_onclick() {
    ...
    if (listContentArry.length > 1 || listSubContentArry.length > 1) {
        ...
        var pTop = (pheight - conHeight) / 2;
        var pLeft = (pwidth - 1200) / 2;
        var pURI = "/ezEmail/new/mailWrite.do?cmd=FORWARD&URL=" + encodeURIComponent(pSelectItem.dataset.href);
...
function Read_StatusChange(pGubun) {
    ...
    if (listContentArry.length > 0) {
        for (var i = 0; i < listContentArry.length; i++) {
            createNodeAndInsertText(xmlpara, objNode, "MESSAGEID", document.getElementById(listContentArry[i]).dataset.href);
        }
    } else if (listSubContentArry.length > 0) {
        for (var i = 0; i < listSubContentArry.length; i++) {
            createNodeAndInsertText(xmlpara, objNode, "MESSAGEID", document.getElementById(listSubContentArry[i]).dataset.href);
        }
    } else {
        createNodeAndInsertText(xmlpara, objNode, "MESSAGEID", currentFixingId.dataset.href);
    }
...
function move_mail_onclick() {
    ...
    let requestUrl = "/ezEmail/new/mailMoveCopy.do";
    ...
    createLayerPopup(400, 600, requestUrl)
        .on("complete", move_mail_onclick_Complete)
        .open();
}

async function move_mail_onclick_Complete(moveUrl) {
    if (typeof (moveUrl) == "undefined")
        return false;

    const isOnlyFixingId = listContentArry.length + listSubContentArry.length === 0;

    if (moveUrl["cmd"] === "MOVE") {
        let includeSecureMail = false;
        for (let i = 0; i < listContentArry.length; i++) {
            if (document.getElementById(listContentArry[i]).dataset.secureMail === "1") {
                includeSecureMail = true;
                break;
            }
        }

        if (includeSecureMail || isOnlyFixingId && currentFixingIdTemp.dataset.secureMail === "1") {
           if (!await confirmPopup(strLangLHM20)) {
               return false;
           }
        }
        ...
        let szItemID = "";
        for (let i = 0; i < listContentArry.length; i++) {
            szItemID += document.getElementById(listContentArry[i]).dataset.href + ",";
        }
        for (let i = 0; i < listSubContentArry.length; i++) {
            szItemID += document.getElementById(listSubContentArry[i]).dataset.href + ",";
        }

        if (isOnlyFixingId) {
            szItemID = currentFixingIdTemp.dataset.href + ",";
        }
    ...
    if (xmlhttp_mailCopy != null && xmlhttp_mailCopy.readyState == 4) {
        if (xmlhttp_mailCopy.status >= 200 && xmlhttp_mailCopy.status < 300) {
            ...
            if (pRtnMessage.indexOf("NO COPY processing failed.") > -1) {
                toast(strLang241).error();
            } else {
                MailListRefresh();

                if(event_Mail_CopyPostSend.mode=="MOVE") {
                    prevShow_Clear();
                    toast(MoveMsg).success();
...
function event_xmlhttp_mailMoveDelete_Complete() {
    if (xmlhttp_mailMoveDelete != null && xmlhttp_mailMoveDelete.readyState == 4) {
        if (xmlhttp_mailMoveDelete.responseText.indexOf("NO COPY processing failed.") > -1) {
            ...
            if (event_xmlhttp_mailMoveDelete_Complete.mode == "MOVE")
                toast(strLang52).error();
            else
                toast(strLang131).error();
        }
        prevShow_Clear();
    }
}
function refreshUnreadCount() {
   getTopLayerWindow()?.get_unreadcount();
}

async function deleteWork(bDel) {
    ...
    if (bDel == true || g_szRootFolderName.replace(' ', '') == strLang4) {
        ...
        if (includeSecureMail) {
            if (!await confirmPopup(strLangLHM19)) {
                return;
            }
        } else {
            if (!await confirmPopup(strLang58)) {
                return;
            }
        }
    } else {
        ...
        if (includeSecureMail) {
            ...
            if (!await confirmPopup(strLang59)) {
                return;
            }
        }
    }
...
function receiveCheck_onClick() {
    ...
    if (listContentArry.length == 1) {
        url = document.getElementById(listContentArry[0]).dataset.href;
    }
    else {
        url = document.getElementById(listSubContentArry[0]).dataset.href;
    }

    var requestUrl = "/ezEmail/new/mailReaderList.do?url=" + encodeURIComponent(url);
    ...
    createLayerPopup(720, 550, requestUrl)
        .on("resend", ReSend)
        .open();
}
function listCount(num) {
    pagination.fetchSize = num;
    MailOptionHidden();
    MailListRefresh();
}
...
function reject_onclick() {
    ...
    const rejectArray = listContentArry.length === 0 ? listSubContentArry : listContentArry;
    const params = [];
    params["email"] = [];
    params["link"] = [];
    for (var n = 0; n < rejectArray.length; n++) {
        ...
        xmlhttp_mailMoveDelete = createXMLHttpRequest();
        createNodeInsert(xmlpara, objNode, "DATA");
        const url = document.getElementById(rejectArray[n]).dataset.href;
        createNodeAndInsertText(xmlpara, objNode, "ITEMID", url);

        let requestUrl = "/ezEmail/mailGetFromEmail.do";
        ...
        try {
            ...
            toast(e.description).error();
        }

        const fromEmail = xmlHTTP.responseText;
        params["email"][n] = fromEmail;
        params["link"][n] = "";
    }

    createLayerPopup(600, 540, "/ezEmail/new/mailDenial.do")
        .parameter("params", params)
        .on("complete", reject_onclick_Complete)
        .open();
}
...
// 2024.04.08 한슬기 : 스팸신고 버튼 추가
function spamReport_onclick(){
	// 스팸신고할 메일을 선택하지 않았을 경우
	if (listContentArry.length === 0 && listSubContentArry.length === 0) {
        toast(strLang42).warning(); // 메일을 선택하세요.
        return;
    } else if (listContentArry.length > 1 || listSubContentArry.length > 1) {
		// 메일을 여러 개 선택했을 경우
		toast(strLang44).warning(); // 메일을 하나만 선택하세요.
		return;
	}
	
    const spamReportArray = listContentArry.length === 0 ? listSubContentArry : listContentArry;
    const params = [];
    params["email"] = [];
    params["link"] = [];
    params["url"] = [];
    
    for (var i = 0; i < spamReportArray.length; i++ ){
		const xmlHTTP = new XMLHttpRequest();
        const xmlpara = createXmlDom();
        let objNode;
        ...
        xmlhttp_mailMoveDelete = createXMLHttpRequest();
        createNodeInsert(xmlpara, objNode, "DATA");
        const url = document.getElementById(spamReportArray[i]).dataset.href;
        createNodeAndInsertText(xmlpara, objNode, "ITEMID", url);
        ...
        const fromEmail = xmlHTTP.responseText;
        params["email"][i] = fromEmail;
        params["link"][i] = "";
        params["url"][i] = url;
	}
    
    // 팝업 호출
    createLayerPopup(480, 325, "/ezEmail/new/spamReport.do")
        .parameter("params", params).open();
...
function prevShow() {
    ...
    try {
        if (listContentArry.length == 0 && listSubContentArry.length == 0) {
            ...
            var Preview_Href;
            if (listContentArry.length > 0) {
                var shareId = document.getElementById(listContentArry[listContentArry.length - 1]).dataset.shareId;
                PrevViewFormH.shareId.value = shareId;
                Preview_Href = document.getElementById(listContentArry[listContentArry.length - 1]).dataset.href;
                xmlhttp_mailPreviewObject = document.getElementById(listContentArry[listContentArry.length - 1]);
            }
            else {
                Preview_Href = document.getElementById(listSubContentArry[listSubContentArry.length - 1]).dataset.href;
            ...
            // ifrmPreViewH.document.body.innerHTML = "";
            PrevViewFormH.iptURL.value = Preview_Href;
            PrevViewFormH.submit();

            $("#MailListRayer").css("width", `${pPreviewHList}%`);
            $("#PreviewRayerH").show();
            resizeResponsivePreview();
...
function event_xmlhttp_mailPreview_Complete() {
    if (xmlhttp_mailPreview != null && xmlhttp_mailPreview.readyState == 4) {
        if (xmlhttp_mailPreview.status >= 200 && xmlhttp_mailPreview.status < 300) {
            ...
            // if (pPreviewShow_HOW == "H") {
            document.getElementById("PreH_subject").dataset.itemId = pItemid;
            document.getElementById("PreH_subject").dataset.contentClass = pContentClass;
            ...
            document.getElementById("PreH_subject").innerHTML = pSubject;
            document.getElementById("PreH_subject").title = pSubject;
            //kms
            pSubject = pSubject.trim();
            if(pSubject == ""){
                document.getElementById("PreH_subject").innerHTML = strLang97;
            }
...
function MailList_ChangeStatus(obj) {
    if (obj.dataset.read === "0") {
        if (obj.dataset.contentClass === "IPM.Note") {
            ...
            const unreadImgSpan = obj.querySelector(".icon_msg_unread");
            unreadImgSpan.classList.replace("icon_msg_unread", "icon_msg_read");
        }
        obj.classList.add("read");
        obj.dataset.read = "1";
        try{
            parent.get_unreadcount();
        }catch(e){}
    }
}
...
function show_personinfo(email) {
    ...
    newDivPopUpShow(650, 670, "/ezCommon/new/showPersonInfo.do?email=" + encodeURIComponent(email));
}

function show_personinfoById(id) {
    newDivPopUpShow(650, 670, "/ezCommon/new/showPersonInfo.do?id=" + encodeURIComponent(id));
}
...
function MailReadOpen() {
    ...
    /*if (pPreviewShow_HOW == "W") {
        ...
        pContentclass = document.getElementById("PreW_subject").dataset.contentClass;
    }
    else {*/
        pItemID = document.getElementById("PreH_subject").dataset.itemId;
        pContentclass = document.getElementById("PreH_subject").dataset.contentClass;
    //}
...
function callMsgDlg(szContentClass, href) {
    ...
    var isDraft = href.includes("Drafts");
    var pheight = window.screen.availHeight;
    var conHeight = 720;
    var pwidth = window.screen.availWidth;
    var conWidth = pwidth * 0.8;
    ...
    if (!isDraft) {
        var pURI = "/ezEmail/new/mailRead.do?iptURL=" + encodeURIComponent(href) + "&PNFlag=Y&CONTENTCLASS=" + encodeURIComponent(szContentClass);
        ...
        if (typeof(shareId) != "undefined" && shareId != "") {
            if (sendPermission == "Y") {
                var pURI = "/ezEmail/new/mailWrite.do?cmd=EDIT&URL=" + encodeURIComponent(href) + "&shareId=" + encodeURIComponent(shareId);
                ...
                toast(strLangKSA02).error();
            }
        } else {
            var pURI = "/ezEmail/new/mailWrite.do?cmd=EDIT&URL=" + encodeURIComponent(href);
...
function callMsgDlgAppr(href) {
    ...
    var pURI = "/ezEmail/new/mailApprRead.do?URL=" + encodeURIComponent(href);
...
async function mail_export() {
    var type = "MAIL";

    if (listContentArry.length == 0 && listSubContentArry.length == 0 && searchMode) {
        if (!await confirmPopup(strLangLS02)) {
            return;
        } else {
            if (!await confirmPopup(strLangLS03)) {
                return;
            } else {
                try {
        ...
        if (!await confirmPopup(strLangLS01)) {
            return;
        } else {
            try {
                mailbox_export();
...
function mailExport_start(pwd){
    ...
    if (PcSaveArrayList.length == 1 && encryptPw == "") {
        var parameters = "url=" + encodeURIComponent(PcSaveArrayList[0].dataset.href);
        ...
        for (var i = 0; i < PcSaveArrayList.length; i++) {
            var folderIdAndMessageId = PcSaveArrayList[i].dataset.href.split("/");
...
function searchedMailExportZip() {
    //var HeaderObject = document.getElementById("MailHeader");
    var ContentObject = document.getElementById("mail-list");
    var pOrderyOption = p_ListorderType + " ORDER BY \"" + p_ListOrderby + "\" " + p_ListOrderOption;
    var attachStatus = "all";
    var andorStatus = "and";
    var end = parseInt(pagination.totalCount);
    var secureMailFilter = document.getElementById("list-type-select").dataset.value == "SECUREMAIL" ? 1 : 0;
    var maxCount = parseInt(pagination.totalCount);
    ...
    var jsonData = {"FOLDERID" : window.tagName ? '' : g_moveUrl,
        ...
        "VIEWSELECTINDEX" : document.getElementById("filter-select").dataset.selectedIndex,
        "LINESELECTINDEX" : document.getElementById("list-type-select").dataset.selectedIndex,
        "END" : end.toString(),
...
function ContextMenuHidden() {
    ...
    if (parent.document.getElementById("folderMenuDiv").style.display == "") {
        parent.document.getElementById("folderMenuDiv").style.display = "none";
    }
...
// 2024-02-14 이사라 - left 메뉴 클릭 시 ContextMenuDiv 숨김처리 
function ContextMenuHidden2() {
	try {
	    if (contentframe.ContextMenuDiv.style.display == "") {
		    contentframe.ContextMenuDiv.style.display = "none";
...
function FolderMenuHidden(){
    if (document.getElementById("folderMenuDiv").style.display == "") {
        document.getElementById("folderMenuDiv").style.display = "none";
    }
}

function PopUpPreMail() {
    const isMainList = _RowObject != null;
    const currentObject = isMainList ? _RowObject : _SubRowObject;

    if (isMainList) {
        const currentIndex = parseInt(currentObject.id.replace("mail-", ""));
        const currentPage = pagination.page;

        if (currentPage === 1 && currentIndex === 0) {
            return "PREEND";
        } else if (currentPage !== 1 && currentIndex === 0) {
            goToPageByNum(currentPage - 1);
            setTimeout(function () { openMailReadWindow(pagination.fetchSize - 1); }, 1000);
            return "PREMOVE";
        } else {
            const preCurrentObject = document.getElementById("mail-" + (currentIndex - 1));
            _RowObject = preCurrentObject;
            callMsgDlg(preCurrentObject.dataset.contentClass, preCurrentObject.dataset.href);
        }
    } else {
        const currentIndex = parseInt(currentObject.id.replace("mail-", ""));
...
function openMailReadWindow(index) {
    const preCurrentObject = document.getElementById("mail-" + index);
    _RowObject = preCurrentObject;
    callMsgDlg(preCurrentObject.dataset.contentClass, preCurrentObject.dataset.href);
}
...
function PopUpNextMail() {
    ...
    if (isMainList) {
        ...
        if (currentPage === pagination.totalPage
            && (currentIndex === pagination.fetchSize - 1 || currentIndex === pagination.totalCount % pagination.fetchSize - 1)) {
            return "NEXTEND";
        } else if (currentPage < pagination.totalPage && currentIndex === pagination.fetchSize - 1) {
            goToPageByNum(currentPage + 1);
            setTimeout(function () { openMailReadWindow(0); }, 1000);
            return "NEXTMOVE";
        } else {
            const preCurrentObject = document.getElementById("mail-" + (currentIndex + 1));
            _RowObject = preCurrentObject;
        ...
        if (currentIndex === pagination.fetchSize - 1) {
            return "NEXTEND";
        } else {
            const preCurrentObject = document.getElementById("mail-" + (currentIndex + 1));
            _SubRowObject = preCurrentObject;
...
function toggle_flag() {
    ...
    if (listContentArry.length > 0) {
        if (listContentArry.length > 1) {
            pSelectItem = "";
            for (var i = 1; i <= listContentArry.length; i++) {
                pSelectItem += document.getElementById(listContentArry[listContentArry.length - i]).dataset.href + ";";
            }
        }
        else
            pSelectItem = document.getElementById(listContentArry[listContentArry.length - 1]).dataset.href + ";";
    } else if (listSubContentArry.length > 0) {
        pSelectItem = document.getElementById(listSubContentArry[listSubContentArry.length - 1]).dataset.href + ";";
    } else {
        pSelectItem = currentFixingId.dataset.href + ";";
    }
...
function itemSingleDelete(obj) {
    var tempM = "BMOVE"
    try {
        if (g_foldertype == "delete") tempM = "BDELETE";
    } catch (e) { }
    Mail_MoveDeletePostSend(tempM, "", $(obj).closest("[id^=mail-]").attr("data-href"));
}
function itemSingleFlag(obj) {
    var now = new Date();
    now.setDate(now.getDate() + 1);
    var month = parseInt(now.getMonth()) + 1;
    var pSDate = now.getFullYear() + "-" + month + "-" + now.getDate();
    ...
    var objNode;
    createNodeInsert(xmlDom, objNode, "DATA");
    createNodeAndInsertText(xmlDom, objNode, "ITEMID", $(obj).closest("[id^=mail-]").attr("data-href"));
    createNodeAndInsertText(xmlDom, objNode, "STARTDATE", pSDate);
    createNodeAndInsertText(xmlDom, objNode, "ENDDATE", pEDate);

    try {
        var shareId = $(obj).closest("[id^=mail-]").attr("data-share-id");
        if (typeof(shareId) != "undefined" && shareId != "") {
            flagXmlHttp.open("POST", "/ezEmail/mailSetFlag.do?shareId=" + encodeURIComponent(shareId), true);
        } else{
...
function cancel_send() {
    ...
    var objNode;
    createNodeInsert(xmlDom, objNode, "DATA");
    createNodeAndInsertText(xmlDom, objNode, "URL", pSelectItem.dataset.href);
        ...
        switch (szStatus) {
            case 200:
                if (g_xmlHttp.responseText == "OK")
                    ...
                    toast(g_xmlHttp.responseText).error()
                }
                break;
        }
    }
...
/*function PreviewMode_ChangeBtn() {
    // TODO 미리보기 버튼 이미지 변경하기
    // document.getElementById("preview-non-img").className = "newSubicon newSubicon032";
...
function MailOptionHidden() {
    commonUi.closeTogglePopup(document.getElementById("layer_popup"));
    ContextMenuHidden();
}
...
function mailConfirm_flag_btn() {
    ...
    if (listContentArrLen > 0) {
        for (var i = 0; i < listContentArry.length; i++) {
            pSelectItem += document.getElementById(listContentArry[i]).dataset.href + ";";
        }
    } else if (listSubContentArry.length > 0) {
        pSelectItem = document.getElementById(listSubContentArry[listSubContentArry.length - 1])
    } else {
...
function onEnterPreviewTagInput() {
    ...
    if ($.grep(document.querySelectorAll("#" + idPrefix + "view > span"), function(span) { return span.innerText == tagName }).length > 0) {
        toast(strLangTagAlreadyUse).warning();
        return;
    }
...
function getPage(status) {
	var num = 1;
	var currentNum = Number(document.getElementById("pageNum").value);
	...
	if(pageTotalNum < currentNum) {
		currentNum = pageTotalNum;
	}

	if("prev" === status) {
		num = currentNum - 1 < 1 ? 1 : (currentNum - 1);
	}

	if ("next" === status) {
		num = currentNum + 1 > pageTotalNum ? pageTotalNum : (currentNum + 1);
	}
