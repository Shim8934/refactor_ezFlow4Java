...
function MailToMe_Onclick() {
    ...
    const spanArray = msgDiv.querySelectorAll("span.spanText");
    ...
    if (checked) {
        ...
        for (const span of spanArray) {
            if (span.getAttribute("email") && span.getAttribute("email") === g_myemail) {
                ...
                msgDiv.removeChild(span.parentElement.parentElement);
            }
        }
    }
}
...
function Send_onClick() {
    if (eSubject.value.trim() == "") {
        toast(strLang92).warning();
        eSubject.focus();
        return;
    }
    
    if (eSubject.value.length > 120) {
        toast(strLang95).warning();
        eSubject.focus();
        return;
    }
    
	if (window.dadiframe && dadiframe.isfileup) {
        toast(strLang86).warning();
		return;
	}
...
var secureMail_dialogArguments = [];
var secureMailParams = [];
function Send_onClick_Complete(ReturnValue) {
    try {
        if (ReturnValue) {
            if (document.querySelectorAll("#MsgToGot span.drop_list_write").length === 0 &&
                document.querySelectorAll("#MsgCCGot span.drop_list_write").length === 0 &&
                document.querySelectorAll("#MsgBCCGot span.drop_list_write").length === 0) {
                toast(strLang93).warning();
            ...
            if (useSecureMail == "YES" && isSecureMail == "true") {
            	...
            	newDivPopUpShow(730, 368, "/ezEmail/new/mailSecureOption.do");
            } else {
...
async function Save_onClick(savemode) {
	...
	if (savemode == "sendsave" && typeof (g_apprMail) != "undefined" && !g_apprMail) {
		if (!await apprPolicy(savemode)) { // 정책에 걸리지 않으면 1
			if (!g_apprMail) {
				return;
			}
		}
...
async function Save_onClick_Complete(ReturnValue) {
    try {
        if (ReturnValue) {
            ...
            if (document.getElementById("bodyType") != null && document.getElementById("bodyType").dataset.value == "1")
            ...
            createNodeAndInsertText(xmlDoc, rootNode, "IMPORTANCE", importantSelect.dataset.selectedIndex);
            ...
            if (m_rgParams4PostOption["delaySendDate"] === "") {
                ...
                if (Save_onClick_Complete.savemode == "sendsave") {
                    ...
                    await alertPopup(strLang98 + "'" + m_rgParams4PostOption["delaySendDate"] + "'" + strLang102 + "'" + strLang100 + "'" + strLang101);
                }
            }
            ...
            // 임시저장 시 첨부파일 삭제 컨펌창
            if (Save_onClick_Complete.savemode == "tempsave" && !isAutoSave && useDeleteAttachmentsFromTempMail) {
                if (await confirmPopup(strLangTempSaveAttachmentsWarning)) {
                    dadiframe.deleteAllAttaches();
                } else {
                    g_saveHttp = null;
                    MailStatus = "NO";
                    isAutoSave = false;
...
//                	window.opener.MailListRefreshByTimeout();
                	...
                	} else if (m_rgParams4PostOption["delaySendDate"] !== "") {
                        // 예약발송 수정
...
function GetMailAddresses(name) {
    m_addrBook = { "type": [], "name": [], "email": [], "href": [], "company": [], "dept": [], "title": [] };
...
function CheckMailReceiver(newElem) {
    const newElemChild = newElem.querySelector("span.spanText");
    const newEmail = newElemChild.getAttribute("email");
    const newHref = newElemChild.getAttribute("href");
    const newGroupHref = newHref ? newElemChild.getAttribute("href").split("|!|")[0] : "";

    for (const wrapper of document.querySelectorAll("#MsgToGot, #MsgCCGot, #MsgBCCGot")) {
        for (const span of wrapper.querySelectorAll("span.spanText")) {
            if (newEmail == span.getAttribute("email") && span.getAttribute("type") != "mailgroup")
                return true;
            else if (newHref && span.getAttribute("type") == "mailgroup") {
                if (newGroupHref == span.getAttribute("href").split("|!|")[0]) {
                    return true;
                }
            }
        }
...
var checkname_cross_dialogArguments = [];
...
function CompleteEmailAddress(formName, validDIV, iType) {
    ...
    for (var i = 0; i < nLen; i++) {
	    ...
	    if (emailExitsCnt == 1) {
	        ...
	        checkname_cross_dialogArguments = [];
            ...
            createLayerPopup(730, 480, "/ezEmail/new/mailCheckName.do")
                .parameter("params", rgParams)
                .on("complete", CompleteEmailAddress_Complete)
                .open();
	        return false;
	    }
    }
...
//	     ofileAttachSize[attachcount] = docfilesize.toString();
    ...
    if (xmlHTTP.status == 200) {
        ...
        if (DocHref.toLowerCase().indexOf(".doc") > 0) {
            ...
            pstrXML += "<ROW><CELL><VALUE><![CDATA[" + getNodeText(GetElementsByTagName(ReturnXML, "DOCTITLE")[0]) + "." + FileExtention + "]]></VALUE>";
            pstrXML += "<DATA1><![CDATA[" + getNodeText(GetElementsByTagName(ReturnXML, "DOCTITLE")[0]) + "." + FileExtention + "]]></DATA1>";
            pstrXML += "<DATA2><![CDATA[" + DocHref + "]]></DATA2>";
            pstrXML += "<DATA3></DATA3>";
...
function ConvertEmbedPath(xmlDoc, rootNode) {
    ...
    if (isBigFile) {
        ...
        if(BigSizeAttachDownloadLimitCount > 0) {
        	var limitCountTmp = BigSizeAttachDownloadLimitCount + strLangHDP01;
        	if (strLangHDP01.indexOf("（）") != -1) {
        		limitCountTmp = strLangHDP01.replace("（）", BigSizeAttachDownloadLimitCount);
        	}
        	TempText += " / <span style='color:#FF0000 ;'>" + limitCountTmp + "</span> " + strLangHDP02;
        }
...
function important_change() {
    ...
    //}
    // m_rgParams4PostOption["important"] = eImportant.value;
    m_rgParams4PostOption["important"] = importantSelect.dataset.selectedIndex;
}

// var letteroption_cross_dialogArguments = [];
function Option_onClick() {
    ...
    var requestUrl = "/ezEmail/new/letterOption.do";
    let height;
    ...
    createLayerPopup(550, height, requestUrl)
        .parameter("params", m_rgParams4PostOption)
        .on("complete", Option_onClick_Complete)
        .open();
}
...
function Option_onClick_Complete(m_rgParams4PostOption) {
 ...
 async function changeTextOption(bodyType) {
    //const selectedBodyType = event.currentTarget.dataset.value;

    if (bodyType === "1") {
        // 이벤트 취소 후 confirm 팝업 띄우기, 이후 확인 이벤트가 들어오면 value 설정 해주기
        //event.preventDefault();

        //if (await confirmPopup("<spring:message code='ezEmail.lhm28' />")) {
            const bodyTypeSelect = document.getElementById("bodyType");
            commonUi.setCustomSelectBoxValueQuietly(bodyTypeSelect, "1");
            ...
            m_rgParams4PostOption["bodyType"] = "1";
            ...
            // 대용량 첨부파일 없애기
            dadiframe.deleteAllBigAttaches();
        //}
    } else {
...
function SelectReceiver_Complete(ReturnValue) {
    ...
    createLayerPopup(1100, 820, "/ezEmail/new/mailNewReceiverChoose.do?defaultwin=" + SelectReceiver_Complete.szDefaultWind)
        .parameter("params", receiverData)
        .on("complete", SelectReceiver_onClick_Complete)
        .open();
...
function SelectReceiver_onClick_Complete(pListViewMsgTo, pListViewMsgCC, pListViewMsgBCC) {
    try {
        ...
        for (const nameSpan of document.querySelectorAll("span.drop_list_write")) {
            nameSpan.parentElement.removeChild(nameSpan);
        }
        ...
        if (pListViewMsgBCC.getElementsByTagName("TR").length > 1) {
            const toggleBtn = document.getElementById("bcc-toggle-btn");
...
function getJustOneReceiverChooseFormat(receiverCol, receiver) {
    ...
    receiverCol.querySelectorAll("span.spanText").forEach((spanName, i) => {
...
function NameChange_onClick() {
    ...
    if (this != null) {
        ...
        createLayerPopup(730, 480, "/ezEmail/new/mailCheckName.do")
            .parameter("params", rgParams)
            .on("complete", NameChange_onClick_Complete)
            .open();
    }
}
...
function NameChange_onClick_Complete(rgParams) {
    ...
    const nameSpan = checkname_cross_dialogArguments[3].querySelector("span.spanText");

    switch (parseInt(nameSpan.getAttribute("iType"))) {
        case 0:
    ...
    if (rgParams["recipientTDData"] == "delete") {
    	decreaseReceiverCount(nameSpan.getAttribute("type"), nameSpan.getAttribute("href"));
        changedReceiverList.removeChild(nameSpan.parentElement.parentElement);
        ...
        for (count1 = 0; count1 < length; count1++) {
        	newElem = PrepareMailTag(nameSpan.getAttribute("iType"), rgParams["returnedRecipientType"][count1], rgParams["returnedRecipientName"][count1],
                    rgParams["returnedRecipientEmail"][count1], rgParams["returnedRecipientHref"][count1]);
        	...
        	if (!IsInsert) {
                ...
                nameSpan.parentElement.parentElement.insertAdjacentElement("afterEnd", newElem);
        	} else {
        		if (!isOrgElemSelected) {
        			if ((nameSpan.getAttribute("type") === "email")
            				&& (rgParams["returnedRecipientType"][count1] === "email")
            				&& (rgParams["returnedRecipientEmail"][count1] === nameSpan.getAttribute("email"))) {
            			isOrgElemSelected = true;
            		} else if ((nameSpan.getAttribute("type") === "mailgroup")
            				&& (rgParams["returnedRecipientType"][count1] === "mailgroup")
            				&& (rgParams["returnedRecipientHref"][count1] === nameSpan.getAttribute("href"))) {
            			isOrgElemSelected = true;
            		}
        		}
        	}
...
function GetAddrFormatForSend(receiveCol) {
    ...
    const emailSpanList = receiveCol.querySelectorAll(".drop_list_write > span.receiverText > span.spanText");

    for (const emailSpan of emailSpanList) {
        switch (emailSpan.getAttribute("type")) {
            case "email":
            case "contact":
                retAddr += "\"" + TrimText(ReplaceText(emailSpan.getAttribute("name").replace('"', "&quot;"), ";", "")) + "\"";
                retAddr += " <" + emailSpan.getAttribute("email") + ">";
                retAddr += ", "
                break;

            case "mailgroup":
                retAddr += GetGroupEmail(emailSpan.getAttribute("href"));
                break;
        }
    }
...
function PrepareMailTag(iWhich, type, name, email, href) {
    ...
    TopSpan.className = "drop_list_write";
    newElem.className = "receiverText";
    ...
    if (g_useAdditionalInfo) {
    	$.ajax({
    		...
    		success	: function(additionalInfo) {
    			...
    			if (type == "mailgroup") {
                    newElem.innerHTML = `<span class='spanText' title='${strLang126}' alt='${strLang126}' href='${href}'>${name}${additionalInfo}</span>`;
                    newElem.innerHTML += "<span class='icon16 elimination' onclick='deleteMailUser(\"" + type + "\",\"" + iWhich + "\",\"" + href + "\")'></span>";
                } else {
    				newElem.innerHTML = "<span class='spanText' title=\"" + email + "\" alt=\"" + email + "\" >" + name + additionalInfo + "</span>";
    				newElem.innerHTML += "<span class='icon16 elimination' onclick='deleteMailUser(\"" + email + "\",\"" + iWhich + "\")'></span>";
    			}

                const nameSpan = newElem.firstElementChild;
                nameSpan.style.cursor = "pointer";
                nameSpan.setAttribute("iType", iWhich);
                nameSpan.setAttribute("onclick", "NameChange_onClick()");
                nameSpan.setAttribute("type", type);
                nameSpan.setAttribute("name", name);
                nameSpan.setAttribute("email", email);
    		},
    		error	: function(error) {
    			console.log(error);
    		}
    ...
    if (type == "mailgroup") {
        ...
        if (!g_useAdditionalInfo) {
            newElem.innerHTML = `<span class='spanText' title='${strLang126}' alt='${strLang126}' href='${href}'>${name}</span>`;
            newElem.innerHTML += "<span class='icon16 elimination' onclick='deleteMailUser(\"" + type + "\",\"" + iWhich + "\",\"" + href + "\")'></span>";
        }
    } else {
        ...
        if (!g_useAdditionalInfo) {
            newElem.innerHTML = "<span class='spanText' title=\"" + email + "\" alt=\"" + email + "\" >" + name + "</span>; ";
            newElem.innerHTML += "<span class='icon16 elimination' onclick='deleteMailUser(\"" + email + "\",\"" + iWhich + "\")'></span>";
        }
    }
...
function deleteMailUser(email, iWhich, href) {
    ...
    const spanTextArray = spanWrapper.querySelectorAll("span.spanText");

    for (const span of spanTextArray) {
        if (email === "mailgroup" && span.getAttribute("href") === href) {
            decreaseReceiverCount(email, href);
            spanWrapper.removeChild(span.parentElement.parentElement);
            return;
    ...
    /*switch (iWhich) {
        case "0":
            const spanTextArray = MsgToGot.querySelectorAll("span.spanText");
            for (const span of spanTextArray) {
            	if (email === "mailgroup" && span.getAttribute("href") === href) {
            		decreaseReceiverCount(email, href);
                	MsgToGot.removeChild(span.parentElement.parentElement);
                	return;
...
var exportOption_cross_dialogArguments = [];
...
function mailExportOption_onClick(type) {
    ...
    createLayerPopup(480, 310, "/ezEmail/new/mailExportOption.do?exportType=" + type)
        .on("complete", (type, pwd) => {
            if (type === "MAIL") {
                mailExport_start(pwd);
            } else {
                mailbox_export_start(pwd);
            }
        })
        .open();
...
var importOption_cross_dialogArguments = [];
...
function preMailRead(Href) {
	...
	var pURI = "/ezEmail/new/mailRead.do?iptURL=" + encodeURIComponent(Href) + "&PNFlag=Y&CONTENTCLASS=PREVIEW";
...
async function apprPolicy(savemode) {
	...
	if ("OK" != chkPolicy) {
		// 예약발송은 승인메일을 할 수 없음
		if (m_rgParams4PostOption["delaySendDate"] !== "") {
			alertPopup(strLangAppr04); return;
		}
		
		if ("ALL_HANDS" == chkPolicy) {
			g_apprMailType = "ALL_HANDS";
			
			g_apprMail = (await confirmPopup(strLangAppr05));
		} else {
			g_apprMailType = "NORMAL";
			
            createLayerPopup(600, 600, "/ezEmail/appr/approverSettingPopUp.do")
            .parameter("savemode", savemode)
            .on("complete", apprComplete)
            .open();
		}
	} else {
		return 1;
