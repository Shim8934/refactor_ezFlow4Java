...
function initTreeInfo(p_Flag, p_UserID, p_DeptID) {
	...
	xmlhttp.open("POST","/ezCar/callNodeTreeData.do?flag=" + encodeURIComponent(selectNo), false);
...
function GetTreeBrdsInfo() {
	...
	var car_id = TreeView.getvalue(nodeIdx, "DATA1");
	...
	if (brdGubun == 1)	{ 
		...
		if( brd_url != "" && brd_url != document.location.protocol+"//"){
			...
			var strUrl = "/ezCar/viewCarList2.do?carID=" + car_id + "&accessCode=" + g_AccessCode; 
			strUrl = strUrl + "&carName=" + encodeURIComponent(car_name);
			
			Navigate( strUrl );
		}
	} else { // 차량클릭하였을때
	        strUrl = "/ezCar/carFormList.do?carID=" + car_id + "&accessCode=" + g_AccessCode;
...
function AddSubBrdTree(p_UserID, p_DeptID, p_BrdID, nodeIdx) {
		...
		xmlhttp.open("POST","/ezCar/callNodeTreeData.do",false);
		xmlhttp.send(xmlpara);
