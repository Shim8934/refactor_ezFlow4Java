...
function TreeNode() {
    ...
    function CreateTreeNode(pNodeData, pEndNode, pParentNode, pUseAgency, pNodeClick, pNodeDblClick, pRequestHandler) {
        ...
        for (var i = 0; i < arrNodeData.length; i++) {
            var tagName = arrNodeData[i].tagName.toUpperCase();
            var tagValue = SelectSingleNodeValue(pNodeData, tagName);
        ...
        for (var i = 0; i < deptList.length; i++) {
        	console.log("authDeptId : " + deptList[i].deptId);
        	if (CN == deptList[i].deptId) {
            	treeDiv.setAttribute("ManageFlag", deptList[i].authType);
            	authType = deptList[i].authType;
        	}
        }
