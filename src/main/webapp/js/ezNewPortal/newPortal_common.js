...
function eventSetting(portletId) { //포틀릿 아이디별로 자바스크립트 로드
	...
	case 26 : // 생일자
		...
		.done(function (script, textStatus) {
			try {
				//생일자 조회 기능 연동
				$("#birthNext").on("click", {isNext : true}, getMonthlyBirthday);
				$("#birthPrev").on("click", {isNext : false}, getMonthlyBirthday);
	...
	case 36 : // 유저정보
		...
		.done(function (script, textStatus) {
			try {
				//근태관리 연동
				var useAttitude = document.getElementById("useAttitude").value;
