...
function tr_unselectedAll(pTableID) {
    var oList = document.getElementById(pTableID);
    var rows = oList.getElementsByTagName("tr");
    ...
    for (var i = 0; i < rows.length; i++) {
        ...
        var strID ;
       if(rows[i].id.indexOf(pTableID)>-1)
...
function tr_selectBlock(pRowID, pTableID) {
    var oList = document.getElementById(pTableID);
    var rowObj = document.getElementById(pRowID);
    ...
    cancelSelectTr(rowObj);
//    tr_unselectedAll(pTableID);

    var lastSelectedIndex = "0";
    if (lastSelectedRowID != ""){
    	lastSelectedIndex = document.getElementById(lastSelectedRowID).dataset.rowidx;
    }

    var currentSelectedIndex = rowObj.dataset.rowidx;
    ...
    for (var i = currentIdx; i <= lastIdx; i++) {
        var objTr = oList.querySelector("tr[data-rowidx='" + i + "']");

        if (objTr) {
            $(objTr).addClass("selectTR");
        }
