...
function get_childXML_2017(url, broot, bcount)
{
    if (navigator.userAgent.indexOf('Trident') == -1) {
        var xmlDOM = createXmlDom();
        ...
        var xmlHTTP = new XMLHttpRequest();
        xmlHTTP.open("POST", "/ezCircular/getCircularFolderAllList.do", false);
	    xmlHTTP.send(xmlDOM);
...
//					
//					if (MakeRightField(DispName[j]) != "C" || MakeRightField(DispName[j]) != "M") {
...
function get_childXML_All(url, broot, bcount) {
	return get_childXML_2017(url, broot, bcount);
}
