...
function getAprLinefor(mode, docid) {
    ...
    xmlhttp.open("POST", "/myoffice/ezApproval/ezLine/aspx/GetLineList.aspx", false);
...
function sendmail(to, eSubject, Drafter, pDraftDate, type, opt, isCheck) {
    ...
    Content = strLang511 + " : " + eSubject + "<br>";

    if (type == "SIHANG") {
        Content += strLang512 + " : " + Drafter + "<br>";
    }
    else if (type == "SIMSABANSONG") {
        Content += strLang513 + " : " + Drafter + "<br>";
    }
    else {
        Content += strLang514 + " : " + Drafter + "<br>";
    }
    if (pDraftDate != "")
        Content += strLang515 + " : " + pDraftDate + "<br>";

    if (pDraftDate.indexOf('오전') > -1) {
        pDraftDate = ReplaceText(pDraftDate, "오전", strLang1122);
    }
    if (pDraftDate.indexOf('오후') > -1) {
        pDraftDate = ReplaceText(pDraftDate, "오후", strLang1123);
    }


    if (type == "callback") Subject = strLang516;
    else if (type == "susin") Subject = strLang517;
    else if (type == "daeree") Subject = strLang518;
    else if (type == "bansong") Subject = strLang519;
    else if (type == "opinion") Subject = strLang541;
    else if (type == "hesong") Subject = strLang520;
    else if (type == "approve_complete" && pDraftFlag == "DRAFT") Subject = strLang521;
    else if (type == "approve_complete" && pDraftFlag == "SUSIN") Subject = strLang522;
    else if (type == "SIHANG") Subject = strLang523;
    else if (type == "SIMSABANSONG") Subject = strLang524;
    else if (type == "SIMSAALERT") Subject = strLang525;
    else if (type == "hukyul") Subject = strLang526;
    else Subject = strLang527;
...
function GetNoticeMail(UserID, type) {
    ...
    xmlhttp.open("Post", "/myoffice/ezPersonal/PersonInfo/GetApprovNoticeMail.aspx", false);
    xmlhttp.send(xmlpara);
    ...
    if (objNodes.length <= 0) {
        ...
        s_alertMail     = getNodeText(GetChildNodes(objNodes[0])[0]);  
        s_completeMail  = getNodeText(GetChildNodes(objNodes[0])[1]);  
        s_bansongMail   = getNodeText(GetChildNodes(objNodes[0])[2]);  
        s_callbackMail  = getNodeText(GetChildNodes(objNodes[0])[3]);  
        s_hesongMail    = getNodeText(GetChildNodes(objNodes[0])[4]);  
        SaveSendBoxFlag = getNodeText(GetChildNodes(objNodes[0])[5]);  
    }
...
function getmailaddress(id) {
    ...
    xmlhttp.open("POST", "/Myoffice/ezApproval/aspx/getmailaddress.aspx", false);
    xmlhttp.send(xmlpara);
...
function receiverIDList(preceiveDeptID) {
    ...
    var xmlhttp = createXMLHttpRequest();
    xmlhttp.open("Post", "/myoffice/ezApproval/aspx/GetReceiverList.aspx", false);
    xmlhttp.send(xmlpara);
    ...
    if (objNodes.length != "0") {
        for (var sn = 0; sn < objNodes.length; sn++) {
            ...
            receiverID = getNodeText(objNodes[sn]);
...
function GetDocInfoData(mode, filed) {
    try {
        ...
        var xmlhttp = createXMLHttpRequest();
        xmlhttp.open("Post", "/myoffice/ezApproval/aspx/GetDocInfoMode.aspx", false);
        xmlhttp.send(xmlpara);
...
function getSameOrgHAPYUIDoc(orgID) {
    ...
    var xmlhttp = createXMLHttpRequest();
    xmlhttp.open("Post", "/myoffice/ezApproval/aspx/getSameOrgHAPYUIDoc.aspx", false);
    xmlhttp.send(xmlpara);
...
function SendMailHesong(CurSelRow) {
    ...
    var NextUserEmail = "";
    var pDocTitle = getNodeText(CurSelRow.cells[0]);
    var draftername = "";
    ...
    var objNode;
    createNodeInsert(xmlpara, objNode, "PARAMETER");  
    createNodeAndInsertText(xmlpara, objNode, "DOCID", GetAttribute(CurSelRow, "DATA1"));
    ...
    getOpinionInfo(GetAttribute(CurSelRow, "DATA1"), "APR");
...
function SendMailToCancel(DocID) {
    ...
    var doctitle = getNodeText(pCurSelRow.cells[0]); 
    var dratertname = getNodeText(pCurSelRow.cells[2]);
    var startdate = getNodeText(pCurSelRow.cells[3]);
...
function getHapyuitype(pSelectedRow, orgid) {
    ...
    if (sn != 0) {
        ...
        if (nowMethod == "A03011") { 
            ...
            var doctitle = getNodeText(pSelectedRow.cells[0]); 
            getOpinionInfo(GetAttribute(pSelectedRow, "DATA1"), "APR");
...
function SendMailOpiniontoReceptionist(pGubn) {
    ...
    if (pDraftFlag == "SUSIN" || pDraftFlag == "HABYUI") {
        if (rtnValue[1] == "Receptionist") {
            ...
            var orgID = trim(getNodeText(GetChildNodes(objNodes[0])[2]));
            ...
            var pwriterID_ = trim(getNodeText(GetChildNodes(objNodes[0])[13]));
            var Drafter_ = trim(getNodeText(GetChildNodes(objNodes[0])[14]));
            var pstartdate_ = trim(getNodeText(GetChildNodes(objNodes[0])[11]));
            var pDocTitle_ = trim(getNodeText(GetChildNodes(objNodes[0])[7]));

            if (pstartdate_ == "" || pstartdate_ == undefined || pstartdate_ == null) {
                pstartdate_ = trim(getNodeText(GetChildNodes(objNodes[0])[12]));
            }
            ...
            var NextUser = trim(getNodeText(GetChildNodes(GetChildNodes(objNodes[lastSN])[0])[4]));
            ...
            var pwriterID = trim(getNodeText(GetChildNodes(objNodes[0])[13]));
            var Drafter = trim(getNodeText(GetChildNodes(objNodes[0])[14]));
            var pstartdate = trim(getNodeText(GetChildNodes(objNodes[0])[11]));
            var pDocTitle_ = trim(getNodeText(GetChildNodes(objNodes[0])[7]));

            if (pstartdate == "" || pstartdate == undefined || pstartdate == null) {
                pstartdate = trim(getNodeText(GetChildNodes(objNodes[0])[12]));
            }
...
function SendMailOpiniontoDrafter(pGubn) {
    ...
    if (pDraftFlag == "DRAFT") {
        ...
        if (pDocTitle) {
            ...
            DocTitle = trim(getNodeText(GetChildNodes(objNodes[0])[7]));
        }
        var NextUser = pwriterID;
        sendmail(NextUser, DocTitle, Drafter, pstartdate, "opinion", "", false);
    }
...
function SendMailOpiniontoApproveMember(pGubn) {
    ...
    if (pDraftFlag == "DRAFT") {
        ...
        var pwriterID_ = trim(getNodeText(GetChildNodes(GetChildNodes(objNodes[objNodes.length - 1])[0])[4]));
        var Drafter_ = trim(getNodeText(GetChildNodes(GetChildNodes(objNodes[objNodes.length - 1])[0])[13]));
        var pstartdate_ = trim(getNodeText(GetChildNodes(GetChildNodes(objNodes[objNodes.length - 1])[0])[2]));
        if (pstartdate_ == "" || pstartdate_ == undefined || pstartdate_ == null) {
            pstartdate_ = trim(getNodeText(GetChildNodes(GetChildNodes(objNodes[objNodes.length - 1])[0])[3]));
        }

        for (var i = 0; i < objNodes.length; i++) {
            approveType = trim(getNodeText(GetChildNodes(GetChildNodes(objNodes[i])[0])[11]));
            approveState = trim(getNodeText(GetChildNodes(GetChildNodes(objNodes[i])[0])[12]));


            if ((approveType == "A03001" || approveType == "A03002" || approveType == "A03009" || approveType == "A03008" || approveType == "A03040") && approveState == "A04003") {
                var pwriterID = trim(getNodeText(GetChildNodes(GetChildNodes(objNodes[i])[0])[4]));
                var Drafter = trim(getNodeText(GetChildNodes(GetChildNodes(objNodes[i])[0])[13]));
                var pstartdate = trim(getNodeText(GetChildNodes(GetChildNodes(objNodes[i])[0])[2]));
                var DocTitle;
                if (pDocTitle) {
                    DocTitle = pDocTitle;
                } else {
                    DocTitle = trim(getNodeText(GetChildNodes(GetChildNodes(objNodes[i])[0])[7]));
                }
                var NextUser = pwriterID;
                ...
                if (approveType == "A03001" && trim(getNodeText(GetChildNodes(GetChildNodes(objNodes[i])[0])[0])) == "1" && pGubn == "BANSONG") {
                    var pwriterID = trim(getNodeText(GetChildNodes(GetChildNodes(objNodes[i])[0])[4]));
                    var Drafter = trim(getNodeText(GetChildNodes(GetChildNodes(objNodes[i])[0])[13]));
                    var pstartdate = trim(getNodeText(GetChildNodes(GetChildNodes(objNodes[i])[0])[2]));
                    var DocTitle;
                    if (pstartdate == "" || pstartdate == undefined || pstartdate == null) {
                        pstartdate = trim(getNodeText(GetChildNodes(GetChildNodes(objNodes[i])[0])[3]));
                    }
                    if (pDocTitle) {
                        DocTitle = pDocTitle;
                    } else {
...
function GetEndOrgDocinfoXml(pOrgDocID) {
    ...
    xmlhttp.open("Post", "/myoffice/ezApproval/aspx/getEndDocInfo.aspx", false);
    xmlhttp.send(xmlpara);
