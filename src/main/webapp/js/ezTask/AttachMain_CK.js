...
function btn_AttachDel_onclick()
{
	try
	{
	    ...
	    if (j)
	    {
		    var pInformationContent = "" + strLang39 + "";
		    var Ans = true;
    ...
    try {
        ...
        for (i = 0; i < objAttachNodes.length; i++) {
            ...
            if (is_newfile != "DEL") {
                ...
                objTr.setAttribute("DATA2", unescape(ServerFile));
                objTr.setAttribute("NEWFILE", unescape(getNodeText(GetChildNodes(GetChildNodes(objAttachNodes[i])[0])[5])));
                ...
                objTd2.innerHTML = unescape(getNodeText(GetChildNodes(GetChildNodes(objAttachNodes[i])[0])[0]));
