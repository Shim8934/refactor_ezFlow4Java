...
function datepicker(thisobjid, elobjid) {
    ...
    function put_vtLocalEndDate(oDate) {
        ...
        //날짜,시간 바뀌지 않게 막음
         ...
         if (m_objCurDate[0].getTime() - m_objCurDate[1].getTime() == -30 || m_objCurDate[0] >= m_objCurDate[1])
    ...
    function mfPopPicker(ePicker, event) {
        ...
        (function(ePicker, event) {
            ...
            switch (szPoplocation) {
                ...
                case "bottomright":
                    ePicker.style.display = "";
                    ePicker.style.left = String(event.clientX - eSrc.offsetWidth + m_winDocBody.scrollLeft) + 'px';
                    ePicker.style.top = String(event.clientY - eSrc.offsetHeight + m_winDocBody.scrollTop + 25) + 'px';
                    if (!fIsRoom(iX, iY, 'right')) iX -= m_iCalWidth + m_iBtnWidth;
        ...
        (function(ePicker, event) {
            ...
            switch (szPoplocation) {
                case "topright":
                    ...
                    ePicker.style.left = event.clientX - event.offsetX + eSrc.offsetWidth + m_winDocBody.scrollLeft + 'px';
                    //ePicker.style.top = event.clientY - event.offsetY - eSrc.offsetHeight + m_winDocBody.scrollTop - 80;
                    ePicker.style.top = event.clientY - event.offsetY - eSrc.offsetHeight + m_winDocBody.scrollTop - 60
                    + ((typeof (eSrc.forceMarginTop) != "undefined" && eSrc.forceMarginTop != null) ? parseInt(eSrc.forceMarginTop) : 0) + 'px'; // 20110923 top 에 forcemargin 더함

                    // 20110923 top 값 지정 후에 다시 top 값을 지정하면 잔상남아 거슬려 주석처리
                    //20070604 
                    //if (typeof (eSrc.forceMarginTop) != "undefined" && eSrc.forceMarginTop != null) {
                    //    ePicker.style.top = ePicker.style.top + parseInt(eSrc.forceMarginTop);
                    //}
