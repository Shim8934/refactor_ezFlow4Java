...
var c_xmlhttp = new ActiveXObject("Microsoft.XMLHTTP");
...
function todayonlaod(s_Year, s_Month, s_Date)
{
    ...
    //dom객체 생성 후 sp에 파라미터로 넘겨줄 xml를 만든다.
    var xmldom = new ActiveXObject("Microsoft.XMLDOM");
    
    var objRoot = xmldom.createNode(1, "PARAMETER", "");
    xmldom.appendChild(objRoot);
    
    var objNode = xmldom.createNode(1, "STARTDATETIME", "");
    objNode.text = datanameweek(sz_Year, sz_Month + 1, sz_Date, "YES");
    objRoot.appendChild(objNode);  
    
    var objNode = xmldom.createNode(1, "ENDDATETIME", "");
    objNode.text = datanameweek(sz_Year, sz_Month + 1, sz_Date, "YES");
    objRoot.appendChild(objNode);
    
    var objNode = xmldom.createNode(1, "APP", "");
    objNode.text = "1";
    objRoot.appendChild(objNode);
...
function weekonload(s_Year, s_Month, s_Date)
{
    ...
    var objNode = xmldom.createNode(1, "ENDDATETIME", "");
    objNode.text = datanameweek(weekEndDate.getFullYear(), weekEndDate.getMonth() + 1, weekEndDate.getDate(), "YES");
    objRoot.appendChild(objNode);
...
function tableListControl_Week()
{
    if (c_xmlhttp.readyState == 4 && c_xmlhttp.status == 200) {
        ...
        xmldom.load(c_xmlhttp.responseXML);
        ...
        document.all.divViewHeader.innerText = weekStartDatename + " ~ " + weekEndDatename;
        ...
        for (var i = sz_Date; i <= sz_Date + 6; i++) {
            ...
            if (i <= endMonthday && i > 0) {
                ...
                _mth.innerHTML = "<font size:5>" + D_nowdateTitle + " [" + dayname[countdayname] + "]</font>";
                _mtr.appendChild(_mth);
        ...
        if (title_name.length == 0) {
            document.all.tdDateCalendarViewer.innerHTML = "<div style='padding-top:20px;'>" + strLang265 + "<div>";
        }
        else {
        ...
        for (var j = 0; j < xmldom.getElementsByTagName("appointment").length; j++) {
            ...
            if (s_weekDateSet == e_weekDateSet) {
                var selObj = document.getElementById("Week_" + xmldom.getElementsByTagName("owner_id")[j].text + "_" + xmldom.getElementsByTagName("dsDaytype")[j].text);

                var approveflag_name = "";
                if (xmldom.getElementsByTagName("approveFlag")[j].text == 1)
                    approveflag_name = "icon_02";
                else
                    approveflag_name = "icon_01";

                var Content_Sp_Start = xmldom.getElementsByTagName("dtstart")[j].text.split("T")[1].split(":");
                var Content_Sp_End = xmldom.getElementsByTagName("dtend")[j].text.split("T")[1].split(":");
                var alldayevent = xmldom.getElementsByTagName("alldayevent")[j].text;
                ...
                if (alldayevent == "0")
                    _span.innerText = Content_Sp_Start[0] + ":" + Content_Sp_Start[1] + " ~ " + Content_Sp_End[0] + ":" + Content_Sp_End[1];
                else
                    _span.innerText = strLang126;
                ...
                _span.onmouseover = new Function("onmouse_over(this);");
                _span.onmouseout = new Function("onmouse_out(this);");
                _span.onclick = new Function("idCalendarViewer_OnDoubleClickAppointment2('" + xmldom.getElementsByTagName("number")[j].text + "','" + xmldom.getElementsByTagName("owner_id")[j].text + "','" + xmldom.getElementsByTagName("dtstart")[j].text.split("T")[0] + "','" + xmldom.getElementsByTagName("dtend")[j].text.split("T")[0] + "','" + selObj.parentNode.firstChild.innerText.trim() + "','" + xmldom.getElementsByTagName("writer_id")[j].text + "');");
                _span.innerText = xmldom.getElementsByTagName("subject")[j].text;
                _td.appendChild(_span);
                _tr.appendChild(_td);
                _table.appendChild(_tr);
...
function makeTable(xmldom, pNum, dayType) {
    ...
    _span.onmouseover = new Function("onmouse_over(this);");
    _span.onmouseout = new Function("onmouse_out(this);");
    _span.onclick = new Function("idCalendarViewer_OnDoubleClickAppointment2('" + getNodeText(xmldom.getElementsByTagName("number")[pNum]) + "','" + getNodeText(xmldom.getElementsByTagName("owner_id")[pNum]) + "','" + getNodeText(xmldom.getElementsByTagName("dtstart")[pNum]).split("T")[0] + "','" + getNodeText(xmldom.getElementsByTagName("dtend")[pNum]).split("T")[0] + "','" + selObj.parentNode.firstChild.innerText.trim() + "','" + getNodeText(xmldom.getElementsByTagName("writer_id")[pNum]) + "');");
    _span.innerText = getNodeText(xmldom.getElementsByTagName("subject")[pNum]);
    _td.appendChild(_span);
    _tr.appendChild(_td);
    _table.appendChild(_tr);
...
function tableListControl_today()
{
    if (c_xmlhttp.readyState == 4 && c_xmlhttp.status == 200) {
        ...
        document.all.divViewHeader.innerText = datanameweek(sz_Year, sz_Month + 1, sz_Date, "HEARDER");
        var _Table = document.createElement("TABLE");
        _Table.setAttribute("class", "table_layout");
        var _Tr = document.createElement("TR");
        var _Th = document.createElement("TH");
        _Th.style.width = "250px";
        _Th.innerText = strLang266;
        ...
        for (var j = 0; j < xmldom.getElementsByTagName("appointment").length; j++) {
            if (xmldom.getElementsByTagName("approveFlag")[j].text == "1") {
                var pObjectId = "Day_" + xmldom.getElementsByTagName("owner_id")[j].text;
                var pObjectSP = (parseInt(xmldom.getElementsByTagName("dstartTime")[j].text) / 30) + 1;
                var pObjectEP = (parseInt(xmldom.getElementsByTagName("dendTime")[j].text) / 30);
                ...
                if ((pObjectSPDay == pObjectEPDay && pObjectSP == pObjectEP) && getNodeText(xmldom.getElementsByTagName("alldayevent")[j]) != "1") {
                    ...
                    document.getElementById(pObjectId + "_" + pObjectSP).onmouseover = new Function("onmouse_over_today(this);");
                    document.getElementById(pObjectId + "_" + pObjectSP).onmouseout = new Function("onmouse_out_today(this);");
                    document.getElementById(pObjectId + "_" + pObjectSP).onclick = new Function("idCalendarViewer_OnDoubleClickAppointment2('" + xmldom.getElementsByTagName("number")[j].text + "', '" + xmldom.getElementsByTagName("owner_id")[j].text + "', '" + xmldom.getElementsByTagName("dtstart")[j].text.split("T")[0] + "', '" + xmldom.getElementsByTagName("dtend")[j].text.split("T")[0] + "', '" + document.getElementById(pObjectId + "_" + pObjectSP).parentNode.firstChild.innerText.trim() + "', '" + xmldom.getElementsByTagName("writer_id")[j].text + "');");
                }
                else {
                    ...
                    if (alldayevent != "1") {
                        ...
                        for (var TCnt = pObjectSP; TCnt <= pObjectEP ; TCnt++) {
                            if (TCnt != pObjectSP) {
                                document.getElementById(pObjectId + "_" + TCnt).removeNode(true);
                            }
                            else {
                                ...
                                document.getElementById(pObjectId + "_" + TCnt).onmouseover = new Function("onmouse_over_today(this);");
                                document.getElementById(pObjectId + "_" + TCnt).onmouseout = new Function("onmouse_out_today(this);");
                                document.getElementById(pObjectId + "_" + TCnt).onclick = new Function("idCalendarViewer_OnDoubleClickAppointment2('" + xmldom.getElementsByTagName("number")[j].text + "', '" + xmldom.getElementsByTagName("owner_id")[j].text + "', '" + xmldom.getElementsByTagName("dtstart")[j].text.split("T")[0] + "', '" + xmldom.getElementsByTagName("dtend")[j].text.split("T")[0] + "', '" + document.getElementById(pObjectId + "_" + TCnt).parentNode.firstChild.innerText.trim() + "', '" + xmldom.getElementsByTagName("writer_id")[j].text + "');");
                        ...
                        document.getElementById(pObjectId + "_1").onmouseover = new Function("onmouse_over_today(this);");
                        document.getElementById(pObjectId + "_1").onmouseout = new Function("onmouse_out_today(this);");
                        document.getElementById(pObjectId + "_1").onclick = new Function("idCalendarViewer_OnDoubleClickAppointment2('" + xmldom.getElementsByTagName("number")[j].text + "', '" + xmldom.getElementsByTagName("owner_id")[j].text + "', '" + xmldom.getElementsByTagName("dtstart")[j].text.split("T")[0] + "', '" + xmldom.getElementsByTagName("dtend")[j].text.split("T")[0] + "', '" + document.getElementById(pObjectId + "_1").parentNode.firstChild.innerText.trim() + "', '" + xmldom.getElementsByTagName("writer_id")[j].text + "');");
        ...
        for (var k = 0; k < title_name.length; k++) {
            for (var j = 0; j < xmldom.getElementsByTagName("appointment").length; j++) {
                var s_weekDateSet = dataSetChange(xmldom.getElementsByTagName("dtstart")[j].text.split("T")[0]);
                var e_weekDateSet = dataSetChange(xmldom.getElementsByTagName("dtend")[j].text.split("T")[0]);

                if (title_name[k].split("/")[0] == xmldom.getElementsByTagName("owner_id")[j].text && xmldom.getElementsByTagName("approveFlag")[j].text == 0) {
                    if (DateTrueFalse(TodayDatename, TodayDatename, s_weekDateSet, e_weekDateSet, TodayDatename, "TODAY")) {
                        ...
                        _TD.align = "left";
                        _TD.onselectstart = function () { return false; };
                        _TD.innerHTML = "<img src='/images/OrganTree_cross/ic-Item.gif' style='vertical-align:middle;'>" + title_name[k].split("/")[1] + " [ " + strLang267 + " : " + xmldom.getElementsByTagName("owner_nm")[j].text + "]";
                        _TD.style.verticalAlign = "middle";
                        _Tr2.appendChild(_TD);
                        var alldayevent = getNodeText(xmldom.getElementsByTagName("alldayevent")[j]);
                        if (alldayevent != "1") {
                            for (var i = 0; i < 48; i++) {
                                ...
                                var t_Startdt = xmldom.getElementsByTagName("dtstart")[j].text.split("T")[1];
                                var t_Enddt = xmldom.getElementsByTagName("dtend")[j].text.split("T")[1];
                                ...
                                if (s_Startdt[0] <= (i / 2) && s_Enddt[0] > (i / 2)) {
                                    //var width_td = d3 * 2;
                                    var _TD = document.createElement("TD");
                                    _TD.title_name = strLang267 + " : " + xmldom.getElementsByTagName("owner_nm")[j].text + "&#13;" + strLang268 + " : " + xmldom.getElementsByTagName("dept_name")[j].text;
                                    _TD.align = "center";
                                    ...
                                    _TD.onmouseover = new Function("onmouse_over_today(this);");
                                    _TD.onmouseout = new Function("onmouse_out_today(this);");
                                    _TD.onclick = new Function("idCalendarViewer_OnDoubleClickAppointment2('" + xmldom.getElementsByTagName("number")[j].text + "', '" + xmldom.getElementsByTagName("owner_id")[j].text + "', '" + xmldom.getElementsByTagName("dtstart")[j].text.split("T")[0] + "', '" + xmldom.getElementsByTagName("dtend")[j].text.split("T")[0] + "', '" + title_name[k].split("/")[1] + "', '" + xmldom.getElementsByTagName("writer_id")[j].text + "');");
                                    //_TD.colSpan = d3;
                                    _Tr2.appendChild(_TD);
                                    //i = i + (d3 - 1);
                                }
        ...
        if (tdcount != 0) {
            ...
            document.getElementById("noapproval").style.display = "none";
            document.all.idCalendarViewer2.innerHTML = "";
        }
...
function idCalendarViewer_OnDoubleClickAppointment2(sz_Num, sz_OwnerID, sz_Start, sz_End, sz_BrdName, sz_WriterID) {
    ...
    //p_url = folderOpen_add;
    if (sz_Num != "" && sz_Num != null) {

        ////읽기창////
        //if (pUse_Editor == "TAGFREE")
        //    window.open("/myoffice/ezResource/ResSch/Schedule_add_TFI.aspx?cmd=mod&from=schedule&" + "num=" + sz_Num + "&ownerID=" + sz_OwnerID + "&type=Master&startDate=" + sz_Start + "&endDate=" + sz_End + "&brdName=" + escape(sz_BrdName), "", "left=" + px + ",top=" + py + ",width=" + c_Width + ", height=" + c_Height + ", status = no, toolbar=no, menubar=no,location=no, resizable=1");
        //else
        //    if (pUse_IE11Browser == "CK")
        //        window.open("/myoffice/ezResource/ResSch/Schedule_Add_IE.aspx?cmd=mod&from=schedule&" + "num=" + sz_Num + "&ownerID=" + sz_OwnerID + "&type=Master&startDate=" + sz_Start + "&endDate=" + sz_End + "&brdName=" + escape(sz_BrdName), "", "left=" + px + ",top=" + py + ",width=" + c_Width + ", height=" + c_Height + ", status = no, toolbar=no, menubar=no,location=no, resizable=1");
        //    else
...
//일보기 마우스임팩트
function onmouse_over_today(td)
{
    td.style.cursor = "pointer";
