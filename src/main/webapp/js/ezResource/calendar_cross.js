...
function idCalendarViewer_OnDoubleClickAppointment(event) {
	...
	if (event.command == 'open'){
		...
		if (CrossYN()) {
		    ...
		    window.open(filename + "?cmd=mod&from=schedule&" + "num=" + szNum + "&ownerID=" + szOwnerID + "&type=" + szType + "&startDate=" + startDate + "&endDate=" + endDate + "&brdName=" + encodeURIComponent(brd_NM), "", "width=820, height=700, status = no, toolbar=no, menubar=no,location=no, resizable=1" + feature);
		}
		else {
    ...
    switch (szCmd.toUpperCase()) {
        case "DAY":
            ...
            if (document.getElementById(g_selTDID))
                document.getElementById(g_selTDID).style.backgroundColor = "rgb(233, 241, 244)";
        ...
        case "WEEK":
            ...
            if (document.getElementById(g_selTRID))
                document.getElementById(g_selTRID).style.backgroundColor = "rgb(233, 241, 244)";
            if (g_selTDID != null && g_selTDID != "")
                sDate = new Date(g_selTDID.substring(7, 11), parseInt(g_selTDID.substring(12, 14)) - 1, parseInt(g_selTDID.substring(15, 17)));
