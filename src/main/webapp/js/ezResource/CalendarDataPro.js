...
function CalMonthDataBind(oAppointment) {
    ...
    if (objElm) {
        ...
        oTd.title = oAppointment.oOwner_nm + "(" + oAppointment.oDept_name + ")";
...
function CalWeekDataBind(oAppointment, order) {
    ...
    if (objDivS) {
        ...
        oDiv.title = oAppointment.oOwner_nm + "(" + oAppointment.oDept_name + ")";
...
function showTooltip(nextTo, e, pTime, pSubject, pApproveFlag) {
    ...
    tTh.innerText = pSubject.split("&apos;").join("'");
    tTr.appendChild(tTh);
    ...
    var sSpan = document.createElement("SPAN");
    sLi.setAttribute("onclick", "OnDoubleClickAppointment(document.getElementById(\"" + nextTo.getAttribute("id") + "\"));hideTooltip();");
...
function Schedule_onMouseClick(event, type) {
    ...
    if (event.style.backgroundColor == "") {
        ...
        if (type == "M") {
            ...
            var resDate = document.getElementsByClassName("resource_ok"); //월에 표시된 모든 자원들
            var eventOwnerID = event.getAttribute("num");
            for (var i = 0; i < resDate.length; i++) {
                var bgColor = "#FF9696";
                var checkOwnerID = resDate[i].parentNode.getAttribute("num");
            ...
            var resDate = document.getElementsByClassName("calendar_data_ok"); //주에 표시된 모든 자원들
            var eventOwnerID = event.getAttribute("num");
            for (var i = 0; i < resDate.length; i++) {
                var bgColor = "#FF9696";

                //시간영역
                var checkOwnerID = resDate[i].getAttribute("num");
                if (eventOwnerID == checkOwnerID) {
        ...
        g_szCurrentApptNum = event.getAttribute("num");				
        g_szCurrentApptPNum = event.getAttribute("pnum");				
        g_szCurrentApptOwnerID = event.getAttribute("owner_id");
        g_szCurrentApptWriterID = event.getAttribute("writer_id");
        g_szCurrentApptInsType = event.getAttribute("instancetype");
        g_szCurrentApptGFlag = event.getAttribute("groupflag");
        g_szCurrentApptSDate = event.getAttribute("dtend");
        g_szCurrentApptEDate = event.getAttribute("dtend");
    }
    else {
...
function showTooltip_MouseOver(nextTo, e, pTime, pSubject, pApproveFlag) {
    ...
    tTable.setAttribute("width", "100%");
    tTh.setAttribute("scope", "col");
    tTh.textContent = pSubject.split("&apos;").join("'");
    tTr.appendChild(tTh);
    tTable.appendChild(tTr);
    ...
    //반복이면 반복이라고 표현한다.
    var reFlag = "";
    if (nextTo.getAttribute("instancetype") == "1") {
        reFlag = " (" + strLang572 + ")";
    }
    //하루종일이면
    if (pTime == strLang126) {
        ...
        //sSpan.className = "width_16";
        sTd.appendChild(sSpan);
        sTd.innerHTML += "[" + strLang569 + "]<br />" + pTime.split(" ~ ")[0] + reFlag;
        sTr.appendChild(sTd);
        sTable.appendChild(sTr);
        tTd.appendChild(sTable);
        tTr.appendChild(tTd);
        ...
        //sSpan.className = "width_16";
        sTd.appendChild(sSpan);
        sTd.innerHTML += "[" + strLang570 + "]<br />" + pTime.split(" ~ ")[1] + reFlag;
        sTr.appendChild(sTd);
        sTable.appendChild(sTr);
        tTd.appendChild(sTable);
        tTr.appendChild(tTd);
    ...
    //sSpan.className = "width_16";
    sTd.appendChild(sSpan);
    sTd.innerHTML += "[" + strLang571 + "]<br />" + nextTo.getAttribute("title");
    sTr.appendChild(sTd);
    sTable.appendChild(sTr);
    tTd.appendChild(sTable);
    tTr.appendChild(tTd);
