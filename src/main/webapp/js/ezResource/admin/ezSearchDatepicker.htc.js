...
function ezBoardDatepicker(thisobjid, elobjid) {
    ...
    function put_vtLocalEndDate(oDate) {
        ...
        //-------------------------------------------------------------
        // 자영 막음 ( 시작 시간 / 분경시 끝 시간 변경 되는 부분 ) 9/24
        //--------------------------------------------------------------
    ...
    function onDocumentReady() {
        ...
        (function() { // safari
            ...
            {
            mfOutputDate(false,m_rgeInputDate[x],0);
            }	
            for (x=0;x < m_rgeInputTime.length;x++)
            {
            mfOutputDate(false,m_rgeInputTime[x],1);
            }
        ...
        (function() { // IE
            ...
            //      is provided as the default. (avoids external dependency)
            m_rgDayNameLetters = (this.daynameLetters == null || this.daynameLetters == "") ?
							    "S;M;T;W;T;F;S".split(";") : this.daynameLetters.split(";");
            m_iCalFormat = (null == this.pickerDateFormat || "" == this.pickerDateFormat) ?
					    "[MMM] [yyyy]" : this.pickerDateFormat;
            m_rgMonthNamesShort = (null == this.monthnamesShort || "" == this.monthnamesShort) ?
						       "Jan;Feb;Mar;Apr;May;Jun;Jul;Aug;Sep;Oct;Nov;Dec".split(";") :
    ...
    function mfCreateTimepicker() {
        ...
        (function() { // safari
            ...
            eNode.size = '2';
            eNode.style.textAlign = 'right';
            eNode.style.height = '172px';
    ...
    function mfCreateCalendar() {
        ...
        (function() {
            ...
            eTable.style.cssText = "width:7em;height:7em;z-index:10;border-collapse:collapse;display:none;cursor:pointer;position:absolute;font-size:9pt;text-align:center;background-color:window;color:windowtext;cursor:pointer;";
            eTR = eTable.insertRow();
            eTR.style.cssText = 'background-color:menu;color:menutext;';
            eTD = eTR.insertCell();
            eTD.prv = '1';
            ...
            eTR = eTable.insertRow();
            eTR.style.cssText = 'background-color:threeddarkshadow;color:threedhighlight;cursor:default;';
            ...
            m_eCalTitle = m_eCalPopup.rows[0].cells[1];
        }).call(this) :
        (function() {
            // IE
            var eTR, eTD, eNode, x, y;
    ...
    function mfWriteEndtimePicker() {
        ...
        (function() {
            ...
            // 추가된 부분
            m_eEndTimePopup.style.width = "150px";
    ...
    function mfBuildMonth() {
        ...
        (function() {
            ...
            if (m_eCalTitle.iMonth != iM || m_eCalTitle.iFullYear != iY) {
                ...
                iOffset += 10; //first date cell
                //Loop through cells setting date numbers and color for previous month

                // 지난달 일자 색깔지정
                for (x = 10, y = iLastDayPrevMonth - iOffset + 11; x < iOffset; x++, y++) {
                    rgCells[x].innerText = y;
                    rgCells[x].style.backgroundColor = 'threedlightshadow';
                }
                ...
                for (y = 1; x < iLastDayThisMonth + iOffset; x++, y++) {
                    rgCells[x].innerText = y;
                    rgCells[x].style.backgroundColor = 'window';
                }
            ...
            if (m_eCalTitle.iMonth == m_objCurDate[iWhichDate].getMonth() && m_eCalTitle.iFullYear == m_objCurDate[iWhichDate].getFullYear()) {
                ...
                rgCells[x].style.backgroundColor = 'highlight';
                rgCells[x].style.color = 'highlighttext';
    ...
    /*function onClickDatePicker() 
    {
    event.cancelBubble=true;
    var iWhichDate = event.srcElement.offsetParent.INPUTELEMENT.WHICHDATE;
    ...
    if (isNaN(iDate) || "highlight" == thisCell.style.backgroundColor)
    {
    if ("highlight" == thisCell.style.backgroundColor)
    {
    mfUnPopPicker();
    }
    return;
    ...
    if(thisCell.style.backgroundColor != "window")
    {
    ...
    */
        ...
        (function(event) {
            ...
            if (null != thisCell.prv) {
                ...
                if (isNaN(iDate) || "highlight" == thisCell.style.backgroundColor) {
                    ...
                    //-------------------------------------------------------------------------------------------
                    //   금일일 경우가 아닐때.
                    //   (금일은 선택되어있으므로 backgroundColor가  highlight 임으로)   
                    //	 08 / 25 -> KiM Ja Young 
                    //-------------------------------------------------------------------------------------------

                    if (iDate != pDate.getDate()) {
                        if ("highlight" == thisCell.style.backgroundColor) {
                            mfUnPopPicker();
                        }
                        return;
                ...
                //	 08 / 25 -> KiM Ja Young 
                //-------------------------------------------------------------------------------------------
                if (thisCell.style.backgroundColor != "window" && iDate != pDate.getDate()) {
                    if (iDate < 15)	//prev month
                    {
                        m_objCurDate[iWhichDate].setMonth(m_eCalTitle.iMonth + 1, iDate);
                    }
                ...
                //mfOutputDate(true,m_rgeInputDate[1],0); 
                //  8/23 -> KiM Ja Young 
                // ( Start Date 변경시 End Date 값이 선언되어지는 것 막음)
                }
                else if (m_objCurDate[0]>m_objCurDate[1]) //end moved before start
                {
                m_objCurDate[0].setTime(m_objCurDate[1].getTime()-msDateSpan); 
                //mfOutputDate(true,m_rgeInputDate[0],0); // 8/23 -> KiM Ja Young 
                // ( Start Date 변경시 End Date 값이 선언되어지는 것 막음)
                }*/
                mfWriteEndtimePicker.call(this);
                mfUnPopPicker.call(this);
    ...
    function mfPopPicker(ePicker, event) {
        ...
        (function(ePicker, event) { // safari
            ...
            if (eSrc.popupLocation != null && eSrc.popupLocation != "") {
                szPoplocation = eSrc.popupLocation.toLowerCase();
            }
