...
function organtreeview(thisobjid, elobjid) {
    ...
    function make_childHtml(nodeIdx, nodeXML) {
				...
				else
				{
				    if (findchildnodevalue(childNode, "DATA7") == 1) //자원관리에서 1:분류 2:자원 이미지 달리표현하기 2008-09-30
				        ...
				        if (findchildnodevalue(childNode, "DATA15") == 1)
				        	...
				        	nodeHtml += "<span class='sub_iconLNB tree_resource_no'></span>";
				        else
        ...
        (function(nodeIdx, nodeXML) {
            ...
            var childHtml = "<span style='display:block;width:175px;'>";
            ...
            for (var i = 0; i < childLength; i++) {
                ...
                if (childNode.selectSingleNode("NODES") != null) {
                    nodeHtml += ("<span id='" + g_childid + nodeCount + "' style='display:inline-block;'>" + make_childHtml(nodeCount, childNode.selectSingleNode("NODES").selectNodes("NODE")) + "</span>");
                    nodeHtml += "</span>"
                }
                else {
                    nodeHtml += ("<span style='display:none' id='" + g_childid + nodeCount + "'></span>");
...
//IE11때문에 navigator.userAgent.indexOf('rv:11.0') == -1) 추가
        ...
        (function() {
            ...
            iterator(
                ...
                function(node) {
                    ...
                    var nodepathval = node.attributes.getNamedItem("path").value;
            ...
            iterator(
                ...
                function(node) {
                    var nodepathvalue = node.attributes.getNamedItem("path").value;
                    var nodeidxvalue = node.attributes.getNamedItem("idx").value;
            ...
            iterator(
                ...
                function(node) {
                    var nodename = node.nodeName;
                    var nodenamevalue = node.attributes.getNamedItem("name").value;
            ...
            (function() {
                ...
                g_imageHeight = sizenode.attributes.getNamedItem("height").value;
            })();
