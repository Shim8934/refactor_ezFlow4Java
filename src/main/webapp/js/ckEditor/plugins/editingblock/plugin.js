			...
			editor.on( 'afterSetData', function()
				{
					...
					//0.5초 후에 afterSetData 이벤트 발생.//파이어폭스에서 딜레이타임이 필요함.
					if(CKEDITOR.env.gecko)
                    {		
    				    setTimeout(  function()
        			    {
		                    CKEDITOR.fire( 'afterSetData', null, editor );
			            }, 
			            500);	
			        }
			        else
			...
			editor.on( 'mode', function( event )
				{
					...
					{
					 //var element = editor.document.$.getById( 'test' );
								//	var element = editor.document.$.getElementsByTagName( 'TD' )[0];
									//alert(element.id);
									//if(element.id.is("test"))
						          //      element.focus();
