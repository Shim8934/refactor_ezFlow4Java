		...
		function resizeColumn() {
			...
			for ( var i = 0; i < cellsCount; i++ ) {
				...
				CKEDITOR.tools.setTimeout( function( leftCell, leftOldWidth, rightCell, rightOldWidth, tableWidth, sizeShift ) {
					leftCell && leftCell.setAttribute( 'width', pxUnit( Math.max( leftOldWidth + sizeShift, 0 ) ) );
					rightCell && rightCell.setAttribute( 'width', pxUnit( Math.max( rightOldWidth - sizeShift, 0 ) ) );

					// If we're in the last cell, we need to resize the table as well
					if ( tableWidth )
						table.setAttribute( 'width', pxUnit( tableWidth + sizeShift * ( rtl ? -1 : 1 ) ) );
				}, 0, this, [
