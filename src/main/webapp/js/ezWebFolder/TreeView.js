    ...
    function CreateTreeNode(pNodeData, pEndNode, pParentNode, pUseAgency, pNodeClick, pNodeDblClick, pRequestHandler) {
        ...
        */
        ...
        if (pRequestHandler != "" && strIsLeaf.toUpperCase() == "FALSE") {
            ...
            imgNode.onclick = new Function("treeicon_toggle(\"" + this.NodeID + "\", \"" + strTreeID + "\", " + pRequestHandler + ", \"" + imgNode.id + "\");");
        }

        treeDiv.appendChild(imgNode);
        //
        //노드 아이콘 생성
        ...
        treeDiv.appendChild(subImgNode);
        //
        ...
        if (pNodeClick != "") {
            ...
            spnNode.onclick = new Function("node_select(\"" + this.NodeID + "\", \"" + this.NodeName + "\", \"" + strTreeID + "\", " + pNodeClick + ");");
        }
        ...
        if (strIsLeaf == "FALSE") {
            ...
            if (pNodeDblClick != "")
                spnNode.ondblclick = new Function(pNodeDblClick + "(\"" + this.NodeID + "\", \"" + this.NodeName + "\"); treeicon_toggle(\"" + this.NodeID + "\", \"" + strTreeID + "\", " + pRequestHandler + ", \"" + imgNode.id + "\");");
            else
                spnNode.ondblclick = new Function("treeicon_toggle(\"" + this.NodeID + "\", \"" + strTreeID + "\", " + pRequestHandler + ", \"" + imgNode.id + "\");");
        }

        treeDiv.appendChild(spnNode);
    ...
    function GetChildImageNodes(pParentNode) {
        ...
        if (window.DOMParser) {
            ...
            for (var i = 0; i < pParentNode.childNodes.length; i++) {
                if (pParentNode.childNodes[i].tagName.toUpperCase() == "IMG") {
                    elements[idx++] = pParentNode.childNodes[i];
                }
            }
        }
