...
public class EzApprovalAdminServiceImpl implements EzApprovalAdminService {
	...
	public String getContainerToDocStateInfo(String companyID, Locale locale, String lang, int tenantID) throws Exception {
		...
		for (int k = 0; k < apprContInfoVOs.size(); k++) {
			...
			resultXML.append("<DATA1>" + commonUtil.makeListField(apprContInfoVOs.get(k).getDocumentState()) + "</DATA1>");
			resultXML.append("<DATA2>" + commonUtil.makeListField(apprContInfoVOs.get(k).getDocumentCode()) + "</DATA2>");
			resultXML.append("</CELL>");
	...
	public String getFormInfo(ApprFormInfoVO apprFormInfoVO) throws Exception {
		...
		for (int k = 0; k < dlength; k++) {
			...
			for (int h = 0; h < headerList.size(); h++) {
				...
				resultXML.append("<VALUE>" + commonUtil.cleanValue(commonUtil.makeListField(fieldValue)) + "</VALUE>");
				
				if (h == 0) {
					resultXML.append("<DATA1>" + commonUtil.makeListField(docXML.getElementsByTagName("FORMID").item(k).getTextContent()) + "</DATA1>");
					resultXML.append("<DATA2>" + commonUtil.makeListField(docXML.getElementsByTagName("FORMDESCRIPTION").item(k).getTextContent()) + "</DATA2>");
					resultXML.append("<DATA3>" + commonUtil.makeListField(docXML.getElementsByTagName("FORMDOCTYPE").item(k).getTextContent()) + "</DATA3>");
					resultXML.append("<DATA4>" + commonUtil.makeListField(docXML.getElementsByTagName("FORMFILELOCATION").item(k).getTextContent()) + "</DATA4>");
					resultXML.append("<DATA5>" + commonUtil.makeListField(docXML.getElementsByTagName("FORMNAME").item(k).getTextContent()) + "</DATA5>");
					resultXML.append("<DATA6>" + commonUtil.makeListField(docXML.getElementsByTagName("FORMNAME2").item(k).getTextContent()) + "</DATA6>");
				}
		...
		for (int k = dlength - 1; k >= 0; k--) {
			...
			for (int h = 0; h < headerList.size(); h++) {
				...
				if (h == 0) {
					...
					resultXML.append("<DATA2>" + commonUtil.makeListField(docXML.getElementsByTagName("HREF").item(k).getTextContent()) + "</DATA2>");
					resultXML.append("<DATA3>" + commonUtil.makeListField(docXML.getElementsByTagName("WRITERID").item(k).getTextContent()) + "</DATA3>");
					resultXML.append("<DATA4>" + docXML.getElementsByTagName("CONTAINERID").item(k).getTextContent() + "</DATA4>");
					resultXML.append("<DATA5>" + commonUtil.makeListField(docXML.getElementsByTagName("ORGDOCID").item(k).getTextContent()) + "</DATA5>");
					...
					resultXML.append("<DATA8>" + commonUtil.makeListField(docXML.getElementsByTagName("ISPUBLIC").item(k).getTextContent()) + "</DATA8>");
			...
			for (int h = 0; h < headerList.size(); h++) {
				...
				if (h == 0) {
					if (mode.equals("GROUP") || mode.equals("JOIN")) {
						resultXML.append("<DATA1>" + commonUtil.makeListField(docXML.getElementsByTagName("MAINID").item(k).getTextContent()) + "</DATA1>");
					} else {
						resultXML.append("<DATA1>" + commonUtil.makeListField(docXML.getElementsByTagName("SUBID").item(k).getTextContent()) + "</DATA1>");
						resultXML.append("<DATA2>" + commonUtil.makeListField(docXML.getElementsByTagName("MAINID").item(k).getTextContent()) + "</DATA2>");
						resultXML.append("<DATA3>" + commonUtil.makeListField(docXML.getElementsByTagName("DEPTID").item(k).getTextContent()) + "</DATA3>");
						resultXML.append("<DATA4>" + commonUtil.makeListField(docXML.getElementsByTagName("COMPANYID").item(k).getTextContent()) + "</DATA4>");
					}
				}
	...
	public String getSealList(ApprSealInfoVO apprSealInfoVO) throws Exception {
		...
		for (int k = 0; k < apprSealInfoVOs.size(); k++) {
			...
			resultXML.append("<CELL>");
			resultXML.append("<VALUE>" + commonUtil.makeListField(commonUtil.getDateStringInUTC(apprSealInfoVOs.get(k).getDelDate(), apprSealInfoVO.getOffSet(), false)) + "</VALUE>");
			resultXML.append("</CELL>");
	...
	public String getUserDocCount(ApprExcelOutVO apprExcelOutVO, Locale locale, String offset) throws Exception {
		...
		for (int k = 0; k < dlength; k++) {
			...
			for (int h = 0; h < headerList.size(); h++) {
				...
				if (h == 0) {
					resultXML.append("<DATA1>" + commonUtil.makeListField(docXML.getElementsByTagName("APRMEMBERDEPTID").item(k).getTextContent()) + "</DATA1>");
					resultXML.append("<DATA2>" + commonUtil.makeListField(docXML.getElementsByTagName("APRMEMBERID").item(k).getTextContent()) + "</DATA2>");
				}
	...
	public String getAprType(LoginVO userInfo) throws Exception {
		...
		if (apprCodeVOs != null) {
			for (int k = 0; k < apprCodeVOs.size(); k++) {
				if (!apprCodeVOs.get(k).getCode().equals(config.getProperty("appr.staATBuSeuSoonChaHyubJo")) && !apprCodeVOs.get(k).getCode().equals(config.getProperty("appr.staATBuSeuByungRyulHyubJo")) && !apprCodeVOs.get(k).getCode().equals(config.getProperty("appr.staATGamSaBu"))) {
	...
	public String getFormProperty(Locale locale, String companyID, int tenantID) throws Exception {
		...
		for (int k = 0; k < apprFormInfoVOs.size(); k++) {
			...
			for (int h = 0; h < apprFormInfoVOs2.size(); h++) {
				resultXML.append("<ROW>");
				resultXML.append("<ID>" + commonUtil.makeListField(apprFormInfoVOs2.get(h).getId()) + "</ID>");
...
//			apprFormInfoVO.setStatus(xmlDom.getElementsByTagName("STATUS").item(0).getTextContent());
			apprFormInfoVO.setValidations("");
			apprFormInfoVO.setStatus("");
		}
		...
		apprFormInfoVO.setFormFormat(".mht");
	...
	public String saveHwpFormInfo(ApprFormInfoVO apprFormInfoVO, String realPath, Locale locale) throws Exception {
		logger.debug("saveHwpFormInfo started");
		...
		if (!apprFormInfoVO.getFormID().equals("") && !apprFormInfoVO.getFormMHT().equals("")) {
			...
			saveFileName = path + commonUtil.separator + apprFormInfoVO.getCompanyID() + commonUtil.separator + "Form" + commonUtil.separator + apprFormInfoVO.getFormID() + ".hwp";
			...
			try {
				...
				byte[] tempByte = Base64.decodeBase64(apprFormInfoVO.getFormMHT());
				
				for (int k = 0; k < tempByte.length; k++) {
					fileOutputStream.write(tempByte[k]);
				}
		...
		apprFormInfoVO.setYear(commonUtil.getTodayUTCTime("yyyy"));
		apprFormInfoVO.setFormFormat(".hwp");
		apprFormInfoVO.setFormAutoRule("");
		apprFormInfoVO.setFormAutoRuleLine("");
		...
		logger.debug("saveHwpFormInfo ended");
