...
public class MScheduleGWController extends EgovFileMngUtil {
	...
	public JSONObject mScheduleTypeList(@PathVariable String userId, HttpServletRequest request) throws Exception {
		...
		try {
			...
			List<ScheduleSecretaryVO> sList = ezScheduleService.getPublicScheduleSec(userId, primary, tenantID, companyID);
			...
			if (primary.equals("1")) {
				...
				for (ScheduleDeptVO vo : pdList) {
					...
					sb.append("<option value='2;;" + vo.getDeptId() + "'" + ">" + egovMessageSource.getMessage("ezSchedule.t373", locale) + " " + commonUtil.cleanValue(vo.getDeptName()) + "</option>");
				}
				
				if (pCompanyAdmin.equals("Y")) {
					//회사일정
					sb.append("<option value='3;;" + companyID + "'" + ">" + egovMessageSource.getMessage("ezSchedule.t374", locale) + " " + info.getCompanyName() + "</option>");
				}

				// 임원일정
				if (userType.equals("a")) { // 임원 + 비서일 경우
					sb.append("<option value='10;;" + userId + "'" + ">" + egovMessageSource.getMessage("ezSchedule.lyj14", locale) + " - " + commonUtil.cleanValue(info.getUserName()) + "</option>");
					...
					for (ScheduleSecretaryVO vo : sList) {
						...
						if (usage.equals("Y")) {
							sb.append("<option value='10;;" + vo.getSecId() + "'" + ">" + egovMessageSource.getMessage("ezSchedule.lyj14", locale) + " - " + commonUtil.cleanValue(vo.getSecName()) + "</option>");
						}
					}
				...
				if (pCompanyAdmin.equals("Y")) {
					//회사일정
					sb.append("<option value='3;;" + companyID + "'" + ">" + egovMessageSource.getMessage("ezSchedule.t374", locale) + " " + info.getCompanyName2() + "</option>");
				}
	...
	public JSONObject mScheduleInsert(@RequestBody JSONObject jsonParam, HttpServletRequest request) throws Exception {
		...
		try {
	        ...
	        // 개인일정이면서 부서일정등록 여부 Y 로 들어왔을 경우 부서일정에도 추가 요건 
	        if ("Y".equals(addDeptSch) && "1".equals(scheduleType)) {
	        	jsonParam.put("scheduleType", "2");
	        	mScheduleService.insertSchedule(jsonParam, utcStartDate, utcEndDate, info.getTenantId(), realPath, locale, info.getOffSet(), info.getLang()); 
	        }
	...
	public JSONObject mScheduleDelete(@PathVariable String scheduleId, HttpServletRequest request) throws Exception {
		...
		try {
			...
			if ("0".equals(delType) && "3".equals(dateType)) {
	            //일정데이터 삭제
		...
		try {
			...
			if (pubScheCumulerVO.size() > 0) {
				for (int i = 0; i < pubScheCumulerVO.size(); i++) {
					if (!info.getDeptId().equals(pubScheCumulerVO.get(i).getDeptId())) {
						...
						scheCumSchedule.put("typeColor", "#b200ff");
			...
			if (pubScheDeptVO2.size() > 0) {
				for (int i = 0; i < pubScheDeptVO2.size(); i++) {
					...
					scheDeptSchedule.put("typeColor", "#b200ff");
	        ...
	        for (int i = 0; i < groupList.size(); i++) {
	        	...
	        	groupSchedule.put("typeColor", Optional.ofNullable(groupList.get(i).getGroupColor()).orElse("#e9de13"));
	        	groupSchedule.put("typeName", "그룹일정 -" + groupList.get(i).getGroupName());
	        	groupSchedule.put("scheduleType", "7");
	        	groupSchedule.put("ownerId", groupList.get(i).getGroupId());
...
//			result.put("gatherList", gatherList);
	...
	public JSONObject scheduleInvitationStatus(@PathVariable String userId, HttpServletRequest request) throws Exception {
		...
		try {
			...
			for (int i=0; i < scheduleIdList.length; i++) {
				ScheduleInfoVO scheduleInfoVO = ezScheduleService.getScheduleInfo(scheduleIdList[i], info.getOffSet(), info.getTenantId(), info.getCompanyId());
				ezScheduleService.updateAttendant(scheduleIdList[i], attendantId, displayName, displayName2, status, info.getTenantId(), "N", info.getLang(), info.getOffSet());
